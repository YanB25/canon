...
std::vector<size_t> col_x_server_thread_nr;
...
struct BenchConfig
{
                                      ...
                                      size_t task_nr)
    {
        ...
        ret.server_thread_nr = server_thread_nr;
        ret.coro_nr = coro_nr;
...
public:
                                                       ...
                                                       size_t task_nr)
    {
        ...
        auto conf = BenchConfig::get_empty_conf(
            name, server_thread_nr, thread_nr, coro_nr, block_size, task_nr);
...
void reg_result(size_t total_ns, const BenchConfig &conf)
{
    ...
    col_x_server_thread_nr.push_back(conf.server_thread_nr);
                                    ...
                                    const BenchConfig &conf)
{
    ...
    auto tid = patronus->get_thread_id();
    auto dir_id = tid % conf.server_thread_nr;
...

{
    ...
    if (is_master)
    {
        ...
        LOG(INFO) << "[bench] BENCH: " << name << ", thread_nr: " << thread_nr
                  << ", server_thread_nr: " << conf.server_thread_nr
               ...
               bool is_master)
{
    ...
    for (size_t server_thread_nr : {1, 2, 4, 8, 16})
    {
        ...
        {
            auto total_test_times = kTestTimePerThread * server_thread_nr;
            {
                auto configs =
                    BenchConfigFactory::get_rlease_nothing("RPC",
                                                           server_thread_nr,
                                                           thread_nr,
                                                           coro_nr,
                                                           block_size,
...
int main(int argc, char *argv[])
{
    ...
    {
        boost::barrier bar(kMaxServerThreadNr);
        std::vector<std::thread> threads;
        for (size_t i = 0; i < kMaxServerThreadNr - 1; ++i)
        {
            threads.emplace_back([patronus, &bar]() {
...

    {
        ...
        df.load_column<size_t>("x_server_thread_nr",
                               std::move(col_x_server_thread_nr));
