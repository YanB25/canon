                              ...
                              CoroExecutionContext<kMaxCoroNr> &ex)
{
    ...
    for (size_t i = 0; i < test_nr; ++i)
    {
        ...
        else if (true_with_prob(delete_unexist_prob))
        {
            ...
            {
                auto rc = rhh->del(key, trace);
                CHECK_EQ(rc, kOk) << util::pre(trace.kv());
                inserted.erase(key);
                keys.erase(key);
                del_succ_nr++;
            }
            ...
            {
                auto rc = rhh->del(key, trace);
                CHECK_EQ(rc, kNotFound) << util::pre(trace.kv());
                del_fail_nr++;
            }
        }
        else if (true_with_prob(get_exist_prob))
        {
            ...
            key = random_choose(keys);
            CHECK_EQ(key[0], mark) << util::pre(trace.kv());
            trace.set("k", key);
            trace.set("v", value);
            trace.set("op", "get");
            ...
            CHECK_EQ(rc, kOk) << "Tid: " << tid << " getting key `" << key
                              << "` expect to succeed";
            CHECK_EQ(got_value, inserted[key]) << util::pre(trace.kv());
            get_succ_nr++;
        }
        else
        {
    ...
    for (const auto &[k, v] : inserted)
    {
        ...
        trace.set("v", v);
        trace.set("op", "get");
        CHECK_EQ(rhh->get(k, get_v, trace), kOk) << util::pre(trace.kv());
        CHECK_EQ(get_v, v) << util::pre(trace.kv());

        auto trace2 = tm.trace("tear down");
        trace2.set("k", k);
        trace2.set("v", v);
        trace2.set("op", "del");
        CHECK_EQ(rhh->del(k, trace2), kOk) << util::pre(trace2.kv());
    }
