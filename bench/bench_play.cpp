...
public:
                             ...
                             bool master) override
    {
        ...
        size_t add_on = conf.get<size_t>("add_on");
        ...
        *(uint64_t *) field_boundary = inl_field_boundary;
        ...
        [[maybe_unused]] auto [buf_nid, buf_addr] = dsm_->explain_gaddr(gaddr);
        [[maybe_unused]] char *result_buffer = (char *) malloc(io_size);
        while (!token->stop_requested())
        {
            for (size_t i = 0; i < batch_size; ++i)
            {
                ...
                if (op == Op::kRead)
                {
                    for (size_t a = 0; a < add_on; ++a)
                    {
                        dsm->prepare_read(rdma_buffer.buffer,
                                          addr + a * (io_size / add_on),
                                          io_size / add_on,
                                          on_chip,
                                          &ctx);
                    }
...
int main(int argc, char *argv[])
{
    ...
    // f.add_option<size_t>("io_size", {8, 64, 256, 512, 1024, 2048, 4_KB});
    f.add_option<size_t>("io_size", {64, 256, 512, 1_KB, 4_KB});
    f.add_option<size_t>("batch_size", {1});
    f.add_option<size_t>("add_on", {1, 2, 4});
