...
std::vector<std::string> col_x_role;
...
namespace std
{
std::string to_string(enum Role role)
{
    if (role == Role::CN)
    {
        return "CN";
    }
    else if (role == Role::MN)
    {
        return "MN";
    }
    else if (role == Role::MN_offload)
    {
        return "MN_offload";
    }
    else
    {
        LOG(FATAL) << "** Unknown role " << (int) role;
        return "";
    }
}
}  // namespace std
...
void reg_result(size_t total_ns, const BenchConfig &conf)
{
    ...
    col_x_role.push_back(std::to_string(conf.role));
                                    ...
                                    const BenchConfig &conf)
{
    ...
    while (coro_comm.thread_remain_task > 0)
    {
        ...
        if (conf.role == Role::MN || conf.role == Role::MN_offload)
        {
                             ...
                             const BenchConfig &conf)
{
    ...
    for (size_t i = 0; i < coro_nr; ++i)
    {
        coro_comm.workers[i] = CoroCall(
            [patronus, &lat_m, coro_id = i, &coro_comm, is_master, &conf](
                CoroYield &yield)
            {
    ...
    coro_comm.master = CoroCall(
        [patronus, test_times, &work_nr, &coro_comm, coro_nr](CoroYield &yield)
        {
...

{
    ...
    LOG_IF(INFO, is_master)
        ...
        << ", entry_size: " << link_entry_size << ", role: " << conf.role
               ...
               [[maybe_unused]] bool is_master)
{
    ...
    // for (auto role : {Role::MN, Role::MN_offload})
    for (auto role : {Role::MN_offload, Role::MN})
    {
        ...
        for (size_t coro_nr : {1})
        {
            ...
            for (size_t linked_list_length : {100})
            {
                // for (size_t entry_size : {8_B, 4_KB, 2_MB})
                for (size_t entry_size : {4_KB, 2_MB})
...
int main(int argc, char *argv[])
{
...

    {
        ...
        df.load_column<std::string>("x_role", std::move(col_x_role));
