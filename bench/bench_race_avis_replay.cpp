...
// clang-format off
// ./bench.py bench_race_avis_trace --buddy_nr=60 --dsm_gb=160 --cache_gb=30 --cluster="50" --block_mb=2
// clang-format on
...
DEFINE_string(cluster, "50", "The cluster of twitter trace");
...
using HashClientT = bench::TwitterHashClient<kE, kB, kS>;
...
public:
    ...
    void init_client(BLS &bls, const Config &config, CoroContext *ctx)
    {
        ...
        auto &pcls = bls.persistent_coroutine();
        auto request_config =
            config.get<bench::TwitterRequestGenerator::Config>(
                "request_config");
        auto nid = bls.id().node_id;
        auto tid = bls.id().thread_id;
        auto cid = bls.id().coro_id;
        auto client_per_machine = config.coro_nr() * config.thread_nr();
        auto client_id =
            nid * client_per_machine + tid * config.coro_nr() + cid;
        LOG(INFO) << PRE(client_id);
        request_config.client_id = client_id;
        HashClientT::BenchConfig conf{
            .g = bench::TwitterRequestGenerator(request_config),
            .en_validate = false};
                             ...
                             bool) override
    {
        ...
        const auto &request_config =
            conf.get<bench::TwitterRequestGenerator::Config>("request_config");
        CHECK(request_config.limit.has_value());

        const auto limit = request_config.limit.value();
        ...
        auto remain_nr = cls.handle_->remain_nr();
        auto this_epoch_nr = std::min(remain_nr, limit);
        if (is_master)
        {
...
int main(int argc, char *argv[])
{
    ...
    bench::TwitterRequestGenerator::Config insert_config{
        .base_dir = "./twitter-trace",
        .cluster = FLAGS_cluster,
        .client_id = 0,  // TODO
        .insert_only = true,
    ...
    f.add_option<bench::TwitterRequestGenerator::Config>("request_config",
                                                         {insert_config});
