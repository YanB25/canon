                             ...
                             bool) override
    {
        ...
        while (!token->stop_requested())
        {
            if (is_client())
            {
                ...
                if (is_master && op_nr == 50_K)
                {
                    // avis::Config::ins().simulate_crash();
                    // LOG(INFO) << "Crashed";
                    auto ptl =
                        std::make_shared<avis::PTL>(dsm_, server_nid_, nullptr);
                    for (size_t leak_size = 8; leak_size <= 1_M; leak_size *= 2)
                    {
                        auto gc =
                            std::make_shared<avis::GC>(dsm_, ptl, leak_size);
                        gc->recover(32 /* parallel */);
                    }
                }
...
int main(int argc, char *argv[])
{
    ...
    // NOTE: only one coro to make number correct
