...
constexpr static size_t kBurnCnt = 10_M;
...
void client_burn(std::shared_ptr<DSM> dsm, size_t thread_nr)
{
    ...
    for (size_t i = 0; i < thread_nr; ++i)
    {
        threads.emplace_back([dsm, &count = counts[i], server_nid]() {
            ...
            auto rdma_buffer = dsm->get_rdma_buffer(2_MB);
            auto [aligned_buf, aligned_size] =
                ::mem::align_address(rdma_buffer.buffer, rdma_buffer.size, 64);
            CHECK_GE(aligned_size, sizeof(ReqMsg));
            CHECK_GE(aligned_buf, rdma_buffer.buffer);
            auto *send_msg = (ReqMsg *) aligned_buf;
            send_msg->tid = tid;
            send_msg->nid = nid;
            ...
            {
                dsm->unreliable_send((const char *) aligned_buf,
                                     sizeof(ReqMsg),
                                     server_nid,
                                     to_dir_id);
                sent++;
                if (sent % kMessageEachToken == 0)
                {
                    ...
                    do
                    {
                        ...
                        if (recv_nr)
                        {
                            dsm->return_buf_no_cpy(recv_ptrs, recv_nr);
                        }
                    } while (token <= 0);
                    VLOG(1) << "[wait] tid " << tid
    ...
    std::array<Perthread<std::atomic<size_t>>, MAX_MACHINE> &gots)
{
    ...
    auto [aligned_buf, aligned_size] =
        ::mem::align_address(rdma_buf.buffer, rdma_buf.size, 64);
    CHECK_GE(aligned_size, sizeof(RespMsg));
    CHECK_GE(aligned_buf, rdma_buf.buffer);
    memset(aligned_buf, 0, sizeof(RespMsg));
    size_t work_nr{0};
