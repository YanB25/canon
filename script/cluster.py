...
class Cluster:
    ...
    def deploy_sync_async(self, targets: 'list[str]', silent: 'bool'):
        ...
        for node in self.__unique_nodes:
            # 1) dispatch (necessary) executables in build/
            if targets is None or len(targets) == 0:
                all_executables = util.listdir(
                    config.BUILD_DIR, full_path=True)
            else:
                # convert short names to build_path names
                all_executables = [os.path.join(
                    config.BUILD_DIR, name) for name in targets]
            self.dispatch_files_async(
                node, all_executables, config.BIN_DIR, silent)

            util.debug(f"Sync {all_executables}")
            ...
            all_artifacts = util.listdir(self_artifacts_dir, full_path=True)
            self.dispatch_files_async(
                node, all_artifacts, config.ARTIFACTS_DIR, silent)

            # 5) the necessary .so
            jemalloc_lib_dir = os.path.join(
                config.THIRD_PARTY_DIR, "jemalloc/lib")
            jemalloc_libs = util.listdir(
                jemalloc_lib_dir, "*.so*", full_path=True)
            shared_libs = jemalloc_libs
            self.dispatch_files_async(
                node, shared_libs, config.BIN_DIR, silent)
            
            # 6) the twitter trace
            # twitter_trace_dir = config.TWITTER_TRACE_DIR
            # dest_dir = os.path.join(config.BIN_DIR, "twitter-trace")
            # twitter_trace = util.listdir(twitter_trace_dir, "*", full_path=True)
            # self.dispatch_files_async(node, twitter_trace, dest_dir, silent)

            # 7) traces
            traces_dir = config.TRACES_DIR
            dest_dir = config.BIN_DIR
            traces = util.listdir(traces_dir, "*", full_path=True)
            self.dispatch_files_async(node, traces, dest_dir, silent)

    def deploy_sync(self, files: 'list[str]', silent: 'bool'):
        self.deploy_sync_async(files, silent)
        self.__join()
    ...
    def dispatch_files_async(self, ip, local_files: 'list[str]', remote_dir: 'str', silent: 'bool'):
        ...
        '''
        cmd = ['rsync', '-azvhP', *local_files, f"root@{ip}:{remote_dir}"]
        self.execute_cmd_async(cmd, ip, silent)
    ...
    def local_test(self, binary: 'str', params: 'list[str]', silent):
        ...
        targets = [binary]
        self.prepare_benchmark(targets, silent)
    ...
    def __execute_local_benchmark(self, binary: 'str', params: 'list[str]', hooks=[]):
        ...
        # NOTE: disable `unbuffer` for now: it is too slow
        # hooks = ["LD_PRELOAD=./libjemalloc.so.2", "unbuffer"] + hooks
        hooks = ["LD_PRELOAD=./libjemalloc.so.2"] + hooks
        hook_command = ' '.join(hooks)

        redirect_log = f"2>&1 |& tee {log_path}"

        script = f"{hook_command} {executable} {pass_params} --exec_meta='{exec_meta}' --rnic={rnic} --machine_nr={machine_nr} --numa_id={numa_id} --node_id={node_id} {redirect_log}"
    ...
    def prepare_benchmark(self, targets: 'list[str]', silent: 'bool'):
        # 1) simulatneously execute these tasks
        build_p = util.rebuild_async(targets, silent)
        self.kill_all_processes_async(silent)
        ...
        # 2) syncing files: ONLY after util.build_async() finished
        self.deploy_sync_async(targets, silent=True)
    ...
    def benchmark(self, binary: 'str', params: 'list[str]', hooks=[]):
        targets = [binary]
        self.prepare_benchmark(targets, True)
    ...
    def __execute_benchmark(self, binary: 'str', params: 'list[str]', ip: 'str', node_id: 'str|int', numa_id: 'str|int', as_async: 'bool', hooks: 'list[str]'):
        ...
        if as_async:
            ...
            redirect_log = f"2>&1 |& tee {log_path}"
        script = f"{hook_command} {executable} {pass_params} --exec_meta='{exec_meta}' --rnic={rnic} --machine_nr={config.MACHINE_NR} --numa_id={numa_id} --node_id={node_id} {redirect_log}"
    ...
    def cluster_bootstrap(self, silent: 'bool'):
        ...
        installs = ['systemd-coredump', 'expect', 'llvm',
                    'valgrind', 'clang-10', 'clang++-10', 'clang-format-10', 'libboost-all-dev']
        # for severless
        # installs.append('libmagick++-dev')
        installs.append('linux-tools-generic')
        installs.append('libmemcached-dev')
        installs.append('libunwind-dev')
        installs.append('libgoogle-perftools-dev')
        installs.append('ccache')
        cmds = ['apt', 'install', '-y', *installs]
        self.cluster_execute_short_cmd(cmds, False)

        util.state("2) allocate huge pages")

        gb = config.HUGEPAGE_GB
        number = int(gb * 1024 / 2)
        self.cluster_execute_short_cmd(
            ["echo", str(number), "> /proc/sys/vm/nr_hugepages"], silent)
