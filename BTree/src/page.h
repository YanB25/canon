...
public:
    ...
    bool remove(btree *bt,
                entry_key_t key,
                bool only_rebalance = false,
                bool with_lock = true)
    {
        return remove_key(key);
    }

    inline void insert_key(entry_key_t key,
                           char *ptr,
                           int *num_entries,
                           bool flush = true,
                ...
                page *invalid_sibling = NULL)
    {
        ...
        {  // internal node
            ...
            return hdr.sibling_ptr->store(
                bt, NULL, key, right, invalid_sibling);
        }
        ...
        {  // FAIR
            ...
            {  // leaf node
                ...
                {
                    sibling->insert_key(
                        records[i].key, records[i].ptr, &sibling_cnt, false);
                }
            ...
            else
            {
                ...
                bt->btree_insert_internal(
                    NULL, split_key, (char *) sibling, hdr.level + 1);
            }
