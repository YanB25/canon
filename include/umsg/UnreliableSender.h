...
public:
    ...
    UnreliableSendMessageConnection(
        ...
        uint64_t mm,
        size_t mm_size,
        AlignedArray<RdmaContext, kEndpointNr> &ctx,
        size_t send_depth)
        : QPs_(QPs),
          send_cqs_(send_cqs),
          ...
          max_allowed_ongoing(send_depth / kSendBatch - 1)
    {
        ...
        {
            send_mrs_[i] = CHECK_NOTNULL(
                createMemoryRegion((uint64_t) mm, mm_size, &ctxs_[i]));
            lkeys_[i] = send_mrs_[i]->lkey;
        }
    ...
    ~UnreliableSendMessageConnection()
    {
        for (size_t i = 0; i < kEndpointNr; ++i)
        {
            CHECK(destroyMemoryRegion(send_mrs_[i]));
            send_mrs_[i] = nullptr;
        }
    }
...
private:
    ...
    AlignedArray<ibv_mr *, kEndpointNr> send_mrs_{};
    ...
    size_t target_ep_id)
{
    ...
    DCHECK_LE(WRID_PREFIX_UMSG_SEND,
              std::numeric_limits<decltype(compound_uint64_t::u16_1)>::max());
    DCHECK_LE(target_ep_id,
              std::numeric_limits<decltype(compound_uint64_t::u16_2)>::max());
    DCHECK_LE(i_ep_id,
              std::numeric_limits<decltype(compound_uint64_t::u16_3)>::max());
    DCHECK_LE(size,
              std::numeric_limits<decltype(compound_uint64_t::u16_4)>::max());
    auto wrid =
        compound_uint64_t(WRID_PREFIX_UMSG_SEND, target_ep_id, i_ep_id, size)
            .val;
                                                        ...
                                                        size_t target_ep_id)
{
    ...
    //     << "** Sender buffer " << (void *) buf << " not aligned to cache
    //     line.";
    ...
    if (ibv_post_send(QPs_[i_ep_id], &wr, &bad_wr))
    {
        PLOG(FATAL) << "** Send with RDMA_SEND failed. wrid: "
                    << compound_uint64_t(wrid)
...
ssize_t UnreliableSendMessageConnection<kEndpointNr>::poll_cq(size_t ep_id)
{
    ...
    for (size_t i = 0; i < polled; ++i)
    {
        if (unlikely(wc[i].status != IBV_WC_SUCCESS))
        {
            ...
            LOG(FATAL) << "[umsg] sender wc failed. wc: " << wc[i];
        }
        else
        {
            auto wrid = compound_uint64_t(wc[i].wr_id);
            auto prefix = wrid.u16_1;
            auto dir_id = wrid.u16_2;
            auto that_ep_id = wrid.u16_3;
            auto size = wrid.u16_4;
            std::ignore = dir_id;
            std::ignore = size;
            DCHECK_EQ(prefix, WRID_PREFIX_UMSG_SEND);
            ongoing_signals_[that_ep_id]--;
            DCHECK_GE(ongoing_signals_[that_ep_id], 0);
        }
    }
    return polled;
}
