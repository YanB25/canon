...
public:
    UnreliableRecvMessageConnection(
        ...
        AlignedArray<RdmaContext, kEndpointNr> &ctxs)
        : QPs_(QPs), recv_cqs_(recv_cqs), ctxs_(ctxs)
    {
        ...
        {
            recv_mrs_[i] = CHECK_NOTNULL(createMemoryRegion(
                (uint64_t) msg_pool_, kMessageBufferSize, &ctxs_[i]));
            lkeys_[i] = recv_mrs_[i]->lkey;
        }
    }
    ...
    ~UnreliableRecvMessageConnection()
    {
        for (size_t i = 0; i < kEndpointNr; ++i)
        {
            CHECK(destroyMemoryRegion(recv_mrs_[i]));
        }
...
private:
    ...
    void mark_buffer_in_used(size_t th_id, size_t batch_id)
    {
        if constexpr (::config::umsg::kEnableNoCpySafeCheck)
        {
            ...
            DCHECK_LE(batch_buffer_in_used_[th_id][batch_id],
    ...
    void validate_buf_batch_is_safe(size_t th_id, size_t buf_id)
    {
        if constexpr (::config::umsg::kEnableNoCpySafeCheck)
        {
            ...
            CHECK_EQ(cnt, 0)
                << "Detected recv buffer batch in-use. th_id: " << th_id
                << ", buf_id: " << buf_id << ", batch_id: " << batch_id
                << ". Detail: " << util::pre(batch_buffer_in_used_[th_id]);
        }
    }
    ...
    AlignedArray<ibv_mr *, kEndpointNr> recv_mrs_{};
    AlignedArray<uint32_t, kEndpointNr> lkeys_{};
                                                         ...
                                                         size_t batch_id)
{
    ...
    else
    {
        ...
        CHECK_LT(batch_id + 1, kRecvBuffer) << "Current  k " << batch_id;
        CHECK_LT(th_id, kEndpointNr);
    }

    CHECK_LE(WRID_PREFIX_UMSG_RECV,
             std::numeric_limits<decltype(compound_uint64_t::u16_1)>::max());
    CHECK_LE(MAX_MACHINE,
             std::numeric_limits<decltype(compound_uint64_t::u16_2)>::max());
    CHECK_LE(kEndpointNr,
             std::numeric_limits<decltype(compound_uint64_t::u16_2)>::max());
    CHECK_LE(batch_id,
             std::numeric_limits<decltype(compound_uint64_t::u16_4)>::max());
    wr.wr_id = compound_uint64_t(
                   WRID_PREFIX_UMSG_RECV, th_id, 0 /* not used */, batch_id)
                   .val;
}
template <size_t kEndpointNr>
    ...
    for (size_t ep_id = 0; ep_id < kEndpointNr; ++ep_id)
    {
        for (size_t i = 0; i < kPostRecvBufferAdvanceBatch; ++i)
        {
            ...
            DVLOG(V) << "[umsg] posting recvs[" << ep_id << "]["
                     ...
                     << compound_uint64_t(post_wr->wr_id);
        }
    }
                                                             ...
                                                             const ibv_wc &wc)
{
    ...
    {
        PLOG(FATAL) << "[umsg] Failed to handle wc: "
                    << compound_uint64_t(wc.wr_id) << ", status: " << wc.status;
        return;
    }
    auto wrid = compound_uint64_t(wc.wr_id);
    auto prefix = wrid.u16_1;

    DCHECK_EQ(prefix, WRID_PREFIX_UMSG_RECV);
    auto th_id = wrid.u16_2;
    std::ignore = wrid.u16_3;
    auto batch_id = wrid.u16_4;
    ...
    if (likely(ibuf != nullptr))
    {
        ...
        DVLOG(V) << "[umsg] Recved msg size " << actual_size << " from ep_id "
                 ...
                 << cur_idx << ", hash is "
                 << util::pre_hex(CityHash64(buf, actual_size)) << " @"
                 << (void *) buf << ". WRID: " << wrid;
    }
    else
    {
    ...
    msg_desc_t *msg_desc, const ibv_wc &wc)
{
    ...
    if (likely(msg_desc != nullptr))
    {
        ...
        // DLOG_IF(WARNING, ((uint64_t) buf) % 64 != 0)
        //     << "** Receiver buffer " << (void *) buf
