...
public:
    ...
    bool add_to_cache(const InternalPage *page);
...
inline IndexCache::IndexCache(int cache_size)
{
...
inline const CacheEntry *IndexCache::find_entry(const Key &from, const Key &to)
{
...
inline bool IndexCache::add_to_cache(const InternalPage *page)
{
...
inline const CacheEntry *IndexCache::search_from_cache(const Key &k,
                                                       ...
                                                       bool is_leader)
{
    if (is_leader && !delay_free_list.empty())
    {  // try to free a page in the delay-free-list
        auto p = delay_free_list.front();
        if (util::asm_rdtsc() - p.second > 3000ull * 10)
        {
            free(p.first);
...
inline bool IndexCache::invalidate(const CacheEntry *entry)
{
    ...
    {
        free_lock.wLock();
        delay_free_list.push(std::make_pair(ptr, util::asm_rdtsc()));
        free_lock.wUnlock();
        return true;
    }
...
{
    uint32_t seed = util::asm_rdtsc();
    GlobalAddress tmp_addr;
retry:
    auto k = rand_r(&seed) % (1000_MB);
