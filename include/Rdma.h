...
bool modifyQPtoInit(struct ibv_qp *qp, uint8_t port);
...
static WcHandler log_wc_handler = [](ibv_wc *wc)
{
    if (unlikely(wc->status != IBV_WC_SUCCESS))
    {
        LOG(ERROR) << "[wc] Failed status " << ibv_wc_status_str(wc->status)
                   << " (" << wc->status << ") for wr_id "
                   << (uint64_t) wc->wr_id << " at QP: " << wc->qp_num
    ...
    ibv_sge &sg, ibv_send_wr &wr, uint64_t source, uint64_t size, uint32_t lkey)
{
    ...
    // if size == 0 => wr.num_sge == 0, wr.sg_list == nullptr
    // in this way, zero-byte message is issued
    // https://www.rdmamojo.com/2013/09/20/zero-byte-messages/
    if (size)
    {
