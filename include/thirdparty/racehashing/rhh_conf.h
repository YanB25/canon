...
public:
    ...
    static RaceHashingHandleConfig get_unprotected(const std::string name,
                                                   bool force_kvblock_to_match)
    {
        auto c = get_basic(name, force_kvblock_to_match);
        c.bypass_prot = true;
    ...
    static RaceHashingHandleConfig get_mw_protected_tenant(
        const std::string &name, bool force_kvblock_to_match)
    {
        auto c = get_basic(name, force_kvblock_to_match);
        c.read_kvblock.use_mw().wo_expire().no_bind_pr();
        c.alloc_kvblock.with_alloc(hash::config::kAllocHintKVBlock)
    ...
    static RaceHashingHandleConfig get_mw_protected_tenant_with_timeout(
        const std::string &name, bool force_kvblock_to_match, Duration timeout)
    {
        auto c = get_basic(name, force_kvblock_to_match);
        c.bypass_prot = true;
    ...
    static RaceHashingHandleConfig get_rpc(const std::string &name,
                                           bool force_kvblock_to_match)
    {
        auto c = get_unprotected(name, force_kvblock_to_match);
        c.use_rpc = true;
        return c;
    }
    ...
    {
        auto c = get_mw_protected(name, false /* force kvblock to match */);
    ...
    static RaceHashingHandleConfig get_mr_protected(const std::string &name,
                                                    bool force_kvblock_to_match)
    {
        auto c = get_mw_protected(name, force_kvblock_to_match);
    ...
    static RaceHashingHandleConfig get_mr_protected_tenant(
        const std::string &name, bool force_kvblock_to_match)
    {
        auto c = get_mw_protected_tenant(name, force_kvblock_to_match);
        c.read_kvblock.use_mr();
        c.alloc_kvblock.use_mr();
        c.meta.d.use_mr();
