...
public:
    ...
    BucketHandle(GlobalAddress raddr, char *bucket_buf)
        : raddr_(raddr), bucket_buf_(bucket_buf)
    {
    }
    constexpr static size_t max_item_nr()
    {
                      ...
                      util::TraceView trace)
    {
        ...
        if (success)
        {
            ...
            {
                history.current().add(Record{.ma = kUpsert,
                                             .ua = kCAS,
                           ...
                           util::TraceView trace)
    {
        for (size_t i = 1; i < kSlotNr; ++i)
        {
            ...
            // NOTE: race hashing use 8 bits as fp and 8 bits as size
            // Therefore no meta is available.
            auto kvblock_remote_addr = h.ptr();
                                  ...
                                  [[maybe_unused]] util::TraceView trace)
    {
        ...
        return rdma_adpt.rdma_write(raddr_,
                                    rdma_buf.buffer,
                                    sizeof(BucketHeader),
                                    0 /* flag */,
                                    mem_handle);
        ...
        for (size_t i = 1; i < kSlotNr; ++i)
        {
            ...
            if (view_handle.match(fp))
            {
                DLOG_IF(INFO, config::kEnableLocateDebug)
                    << "[race][locate] YES. locate: fp " << pre_fp(fp)
                    << " got matched FP " << pre_fp(fp) << ". view "
                    << view_handle << ". " << util::pre(trace.kv());
                ret.insert(view_handle);
            }
            // else
            // {
            //     DLOG_IF(INFO, config::kEnableLocateDebug)
            //         << "[race][locate] NO. locate: fp " << pre_fp(fp)
            //         << " mismatch with " << view_handle;
            // }
        }
        return kOk;
    }
                               ...
                               util::TraceView trace) const
    {
        ...
        if (expect_ld == h.ld)
        {
            ...
            if (rounded_suffix != rounded_header_suffix)
            {
                DLOG_IF(INFO, config::kEnableDebug)
                    ...
                    << rounded_header_suffix << ". " << util::pre(trace.kv());
                return kCacheStale;
            }
        ...
        LOG(WARNING) << "[race][trace] validate_staleness kCacheStale: "
                        "expect_ld: "
                     << expect_ld << ", expect_suffix: " << suffix
                     << ", rounded to: " << rounded_suffix
                     << ", header_ld: " << (void *) (uint64_t) h.ld
                     << ", header_suffix: " << (void *) (uint64_t) h.suffix
                     << ", rounded to: " << rounded_header_suffix
                     << ". Rounded bit: " << rounded_bit;
        return kCacheStale;
    }
