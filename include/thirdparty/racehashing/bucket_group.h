...
{
public:
    CombinedBucketHandle(GlobalAddress raddr, bool main_on_left)
        : raddr_(raddr), main_on_left_(main_on_left)
    {
    }
    ...
    RetCode read(IRdmaAdaptor &rdma_adpt, RemoteMemHandle &handle)
    {
        ...
        // memset(rdma_buf.buffer, 0xff, size_bytes());
        DLOG_IF(INFO, config::kEnableMemoryDebug)
            << "[race][mem] in bucket_group::read: raddr_: " << raddr_
            << ", buffer: " << (void *) buffer_;
        return rdma_adpt.rdma_read(
            (char *) buffer_, raddr_, size_bytes(), 0 /* flag */, handle);
    }
    ...
    BucketHandle<kSlotNr> main_bucket_handle() const
    {
        if (main_on_left_)
        {
            return BucketHandle<kSlotNr>(raddr_, (char *) buffer_);
        }
        return BucketHandle<kSlotNr>(raddr_ + bucket_size_bytes(),
...
public:
    ...
    {
        return main_bucket_0().total_object_bytes() +
               main_bucket_1().total_object_bytes() +
               overflow_bucket().total_object_bytes();
    }
