...
public:
    ...
    size_t total_object_bytes() const
    {
        ...
        for (size_t i = 0; i < kBucketGroupNr; ++i)
        {
            ret += bucket_group(i).total_object_bytes();
        }
        return ret;
    }
...
public:
    ...
    SubTableHandle(GlobalAddress st_raddr, RemoteMemHandle &st_mem_handle)
        : st_raddr_(st_raddr), st_mem_handle_(st_mem_handle)
    {
        CHECK(!st_raddr_.is_null());
    }
                                         ...
                                         util::TraceView trace)
    {
        ...
        {
            auto bucket_raddr = st_raddr_ + i * Bucket<kSlotNr>::size_bytes();
            auto b = BucketHandle<kSlotNr>(bucket_raddr, nullptr);
    ...
    CombinedBucketHandle<kSlotNr> combined_bucket_handle(size_t idx)
    {
        ...
        if (idx % 2 == 0)
        {
            // even
            idx /= 2;
            GlobalAddress bucket_group_gaddr = st_raddr_ + kItemSize * idx;
            DLOG_IF(INFO, config::kEnableMemoryDebug)
                << "[race][mem] combined_bucket_handle: left. indexing "
                << origin_idx << ", map to idx " << idx
                << ", from kCombinedBucketNr: " << kCombinedBucketNr
                << ", st_raddr: " << st_raddr_ << ", kItemSize: " << kItemSize
                << ", combined_bucket: [" << bucket_group_gaddr << ", "
                << bucket_group_gaddr + kItemSize << ")";
        ...
        else
        {
            ...
            LOG_IF(INFO, config::kEnableMemoryDebug)
                ...
                << origin_idx << ", map to idx " << idx
                << " from kCombinedBucketNr: " << kCombinedBucketNr
                << ", st_addr: " << st_raddr_ << ", kItemSize: " << kItemSize
                << ", combined_bucket: [" << overflow_bucket_gaddr << ", "
                << overflow_bucket_gaddr + kItemSize << ")";
                                ...
                                const SubTable<kBucketGroupNr, kSlotNr> &st)
{
    ...
    auto total_object_bytes = st.total_object_bytes();
    os << "Total object bytes: " << total_object_bytes << " ("
       << util::pre_byte(total_object_bytes) << ") ";
