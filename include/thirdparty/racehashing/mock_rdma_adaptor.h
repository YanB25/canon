...
public:
    ...
    using IAllocator = ::mem::IAllocator;
    ...
    GlobalAddress remote_alloc(size_t size, hint_t hint) override
    {
        ...
        {
            return to_exposed_raddr(it->second->alloc(size));
        }
    ...
    GlobalAddress do_remote_alloc(size_t size, hint_t hint)
    {
        ...
        DLOG_IF(INFO, config::kEnableDebug)
            ...
            << " for size " << size << ". " << util::pre(trace_.kv());
                                 ...
                                 flag_t flag) override
    {
        ...
        if (!gaddr.is_null())
        {
            ...
            return alloc_handle(
                raddr, size, patronus::AcquireRequestStatus::kReserved);
        }
        else
        {
            CHECK(alloc_semantics);
    ...
    GlobalAddress rpc_alloc(size_t size, hint_t hint)
    {
        ...
        auto remote_raddr = to_exposed_raddr(ret);
        DLOG_IF(INFO, config::kEnableDebug)
            << "[rdma][trace] rpc_alloc: " << (void *) ret << "("
            << remote_raddr << ") for size " << size << ". "
            << util::pre(trace_.kv());
        return remote_raddr;
    }
    void rpc_free(GlobalAddress raddr, size_t size, hint_t hint)
    {
        std::lock_guard<std::mutex> lk(rpc_mu_);

        auto *addr = from_exposed_raddr(raddr);
    ...
    void remote_free(GlobalAddress raddr, size_t size, hint_t hint) override
    {
        DLOG_IF(INFO, config::kEnableDebug)
            << "[rdma][trace] remote_free: " << raddr << ". "
            << util::pre(trace_.kv());
        server_ep_.lock()->rpc_free(raddr, size, hint);
    }
                         ...
                         flag_t flag) override
    {
        ...
        {
            remote_free(handle.raddr(), handle.size(), hint);
        }
        else
        {
            // CHECK_EQ(hint, 0);
                      ...
                      RemoteMemHandle &handle) override
    {
        ...
        op.op = 0;
        op.size = size;
        debug_validate_handle(handle, raddr, size);
                     ...
                     RemoteMemHandle &handle) override
    {
        ...
        debug_validate_handle(handle, raddr, 8);
    ...
    {
        auto gaddr = to_gaddr(raddr);
        auto handle_gaddr = to_gaddr(handle.raddr());
        CHECK(handle.valid()) << "** Invalid mem_handle provided: " << handle;
        CHECK_EQ(gaddr.nodeID, handle_gaddr.nodeID)
            << "** Mismatch node detected. gaddr: " << gaddr
            << ", handle.gaddr(): " << handle_gaddr;
        CHECK_GE(gaddr.offset, handle_gaddr.offset)
            << "** Underflow detected. gaddr: " << gaddr << " with offset "
            << gaddr.offset << ", handle.gaddr() " << handle_gaddr
            << " with offset " << handle_gaddr.offset;
        CHECK_LE((void *) ((char *) gaddr.offset + size),
                 (void *) ((char *) handle_gaddr.offset + handle.size()))
            << "** Overflow detected. gaddr: " << gaddr
            << " with offset to the handle base addr: "
            << (uint64_t) gaddr.offset - handle_gaddr.offset << " and r/w size "
            << size << " overflow handle size " << handle.size();
    }
    GlobalAddress to_gaddr(GlobalAddress raddr)
    {
        GlobalAddress ret;
        ret.val = raddr.val;
        return ret;
    }
    GlobalAddress to_raddr(GlobalAddress gaddr)
    {
        GlobalAddress ret;
        ret.val = gaddr.val;
        return ret;
    ...
    void *from_exposed_raddr(GlobalAddress raddr) override
    {
        auto gaddr = to_gaddr(raddr);
...
private:
    RemoteMemHandle alloc_handle(GlobalAddress raddr,
                                 size_t size,
                                 patronus::AcquireRequestStatus ec)
    {
        auto hid = get_handle_id();
        RemoteMemHandle ret(raddr, size, ec);
