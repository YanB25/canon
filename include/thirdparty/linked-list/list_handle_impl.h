...
public:
    ...
    {
        memcpy(&meta_.cache_, rdma_buf.buffer, meta_size());
        meta_.inited_ = true;
    }
...
private:
    ...
    {
        if (unlikely(!meta_.handle_.valid()))
        {
            meta_.handle_ = meta_acquire_perm(meta_.gaddr_, meta_size());
            CHECK(meta_.handle_.valid());
        }
        return meta_.handle_;
    }
    void relinquish_meta_handle()
    {
        if (meta_.handle_.valid())
        {
            meta_relinquish_handle(meta_.handle_);
        }
    }
