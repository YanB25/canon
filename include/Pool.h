...
public:
    ...
    {
        DCHECK(outer_->insert(addr).second);
        DCHECK_EQ(inner_->erase(addr), 1);
    }
    void debug_validity_put(void *addr)
    {
        DCHECK_EQ(outer_->erase(addr), 1)
            << "addr " << addr << " not out-going buffer.";
        DCHECK(inner_->insert(addr).second)
...
public:
    ...
    {
        auto *ret = do_alloc();
        new (ret) T();  // call ctor before return
        return ret;
    }
    T *do_alloc()
    {
...
private:
    ...
    size_t cur_alloc_nr_{8};

    void refill()
    {
        storage_.emplace_back(cur_alloc_nr_);
        cur_alloc_nr_ *= 2;
        for (auto &obj : storage_.back())
        {
            pool_.push(&obj);
        }
    }
