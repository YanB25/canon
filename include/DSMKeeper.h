...
struct ExchangeMeta
{
    ...
    ExUnreliable umsgs[kCorePerNuma];
} __attribute__((packed));
...
public:
                         ...
                         Duration sleep_time)
    {
        ...
        std::chrono::nanoseconds warning_time = 1s;
        auto begin = std::chrono::steady_clock::now();
        uint64_t remember_v = 0;
        while (true)
        {
            auto now = std::chrono::steady_clock::now();
            if (unlikely(now - begin >= warning_time))
            {
                LOG(WARNING)
                    << "[keeper] Barrier for a long time: key: " << barrierKey
                    << " state: " << remember_v << " / " << expect_nr
                    << " . Timeout: " << util::pre(warning_time);
                double scale = 1.5;
                warning_time = std::chrono::nanoseconds(
                    (uint64_t)(warning_time.count() * scale));
                begin = now;
            }
            auto *ret = memGet(
    ...
    std::optional<uint64_t> try_sum(const std::string &sum_key,
                                    uint64_t value,
    ...
    void connectUnreliableMsg(UnreliableConnection<kCorePerNuma> &umsg,
                              int remoteID);
