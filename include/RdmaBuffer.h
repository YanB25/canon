...
public:
    ...
    void set_buffer(Buffer &&buffer)
    {
        ...
        CHECK_LE((uint64_t) entry_buffer, (uint64_t) buffer.buffer_end())
            << "** buffer overflow detected.";
        *zero_64bit = 0;

        CHECK_LT((char *) zero_64bit + 8 - buffer.buffer,
                 define::kPerCoroRdmaBuf);
    }
