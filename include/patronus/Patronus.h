...
struct PatronusConfig
{
    ...
    size_t machine_nr{FLAGS_machine_nr};
...
public:
    ...
    constexpr static size_t kMwPoolSizePerThread =
        ::config::patronus::kMwPoolSizePerThread;
    constexpr static uint64_t kDefaultHint = 0;
    constexpr static size_t kMaxSyncKey = 128;

    using PatronusBatchContextT = PatronusBatchContext<kMaxCoroNr>;
    ...
    [[nodiscard]] inline RetCode rpc_cas(Lease &lease,
                                         ...
                                         TraceView = util::nulltrace);
    // below for batch API
    [[nodiscard]] RetCode prepare_write(PatronusBatchContextT &batch,
                                        Lease &lease,
                                        const char *ibuf,
                                        size_t size,
                                        size_t offset,
    ...
    [[nodiscard]] RetCode prepare_read(PatronusBatchContextT &batch,
                                       Lease &lease,
                                       char *obuf,
                                       size_t size,
                                       size_t offset,
                                       ...
                                       CoroContext *ctx = nullptr,
                                       TraceView = util::nulltrace);
    [[nodiscard]] RetCode prepare_cas(PatronusBatchContextT &batch,
                                      Lease &lease,
                                      char *iobuf,
                                      size_t offset,
                                      uint64_t compare,
                                      ...
                                      CoroContext *ctx = nullptr,
                                      TraceView = util::nulltrace);
    [[nodiscard]] RetCode prepare_faa(PatronusBatchContextT &batch,
                                      Lease &lease,
                                      char *iobuf,
                                      size_t offset,
                                      int64_t value,
    ...
    [[nodiscard]] inline RetCode rpc_faa(Lease &lease,
                                         ...
                                         CoroContext *ctx = nullptr,
                                         TraceView = util::nulltrace);
    [[nodiscard]] RetCode remote_memcpy(size_t times,
                                        size_t size,
                                        size_t nid,
                                        size_t dir_id,
                                        bool offload,
                                        CoroContext *ctx);
    [[nodiscard]] RetCode commit(PatronusBatchContextT &batch,
                                 CoroContext *ctx = nullptr,
                                 TraceView = util::nulltrace);
    ...
    // belows are the unprotected reason: very handy
    [[nodiscard]] RetCode prepare_unprot_write(
        PatronusBatchContextT &batch,
        size_t node_id,
        size_t dir_id,
        const char *ibuf,
        size_t size,
        ...
        CoroContext *ctx = nullptr,
        util::TraceView = util::nulltrace);
    [[nodiscard]] RetCode prepare_unprot_read(PatronusBatchContextT &batch,
                                              size_t node_id,
                                              size_t dir_id,
                                              char *obuf,
                                              size_t size,
                                              ...
                                              CoroContext *ctx = nullptr,
                                              TraceView = util::nulltrace);
    [[nodiscard]] RetCode prepare_unprot_cas(PatronusBatchContextT &batch,
                                             size_t node_id,
                                             size_t dir_id,
                                             char *iobuf,
                                             size_t offset,
                                             ...
                                             CoroContext *ctx = nullptr,
                                             TraceView = util::nulltrace);
    [[nodiscard]] RetCode prepare_unprot_faa(PatronusBatchContextT &batch,
                                             size_t node_id,
                                             size_t dir_id,
                                             char *iobuf,
                                             size_t offset,
...
public:
    ...
    {
        put_rdma_buffer(std::move(buffer));
    }
...
private:
    ...
    static thread_local ssize_t ongoing_rdma_nr_;
    AsyncContext<RDMA> *get_rw_context(CoroContext *ctx)
    {
        auto *ret = new AsyncContext<RDMA>();
        ret->ready = false;
        ongoing_rdma_nr_++;
        if (likely(ctx != nullptr))
        {
            ret->coro_id = ctx->coro_id();
            ret->tid = ctx->thread_id();
            DCHECK_EQ(ctx->thread_id(), get_thread_id());
            ret->coro_state = ctx->coro_state();
            ret->coro_state->expect_resp_type = RpcType::kReserved;
        }
        else
        {
            ret->coro_id = kNotACoro;
            ret->tid = get_thread_id();
            ret->coro_state = nullptr;
        }
        return ret;
    }
    void put_rw_context(AsyncContext<RDMA> *rw_context)
    {
        if (rw_context)
        {
            ongoing_rdma_nr_--;
            DCHECK_GE(ongoing_rdma_nr_, 0);
            delete rw_context;
        }
    }
    ssize_t ongoing_rdma_nr() const
    {
        return ongoing_rdma_nr_;
    }

    static thread_local ssize_t ongoing_rpc_nr_;

    AsyncContext<RPC> *get_rpc_context(CoroContext *ctx)
    {
        auto *ret = new AsyncContext<RPC>();
        ...
        if (likely(ctx != nullptr))
        {
            ...
            DCHECK_EQ(ctx->thread_id(), get_thread_id());
            ret->coro_state = ctx->coro_state();
            ret->coro_state->expect_wr_id = nullwrid.val();
        }
        else
        {
            ret->coro_id = kNotACoro;
    ...
    void put_rpc_context(AsyncContext<RPC> *rpc_context)
    {
        if (rpc_context)
        {
            ongoing_rpc_nr_--;
            DCHECK_GE(ongoing_rpc_nr_, 0);
            delete rpc_context;
        }
    }
    ssize_t ongoing_rpc_nr() const
    {
        return ongoing_rpc_nr_;
    }
    ...
    static thread_local std::unique_ptr<::mem::SlabAllocator>
        rdma_client_buffer_;
    static thread_local char *client_rdma_buffer_;
    static thread_local size_t client_rdma_buffer_size_;
    static thread_local PatronusBatchContextT one_shot_batch_ctx_;
    ...
    // [NR_DIRECTORY]
    static thread_local std::array<std::unique_ptr<mem::MWPool>, NR_DIRECTORY>
        mw_pool_;
...
Lease Patronus::do_get_rlease(uint16_t node_id,
                              uint16_t dir_id,
                              GlobalAddress bind_gaddr,
                              uint64_t alloc_hint,
                              size_t size,
...
// gaddr is buffer offset
Lease Patronus::do_get_wlease(uint16_t node_id,
                              uint16_t dir_id,
                              GlobalAddress bind_gaddr,
                              uint64_t alloc_hint,
                              size_t size,
                                     ...
                                     CoroContext *ctx)
{
    ...
    RetCode ret = RC::kOk;
    auto rdma_buf = get_rdma_buffer(sizeof(AdminRequest));
    DCHECK_GE(rdma_buf.size, sizeof(AdminRequest));
    AsyncContext<RPC> *rpc_context = nullptr;
    ...
    if (need_response)
    {
        rpc_context = get_rpc_context(ctx);
        ...
        rpc_context->inner.ret_code = RC::kReserved;
    }
    ...
    if (need_response)
    {
        DCHECK_NOTNULL(ctx)->record_yield_reason(rpc_context,
                                                 (BaseMessage *) msg);
        ctx->yield_to_master();
        ...
        ret = rpc_context->inner.ret_code;
                                 ...
                                 TraceView trace)
{
    ...
    rpc_context->inner.request = (BaseMessage *) msg;
    rpc_context->inner.ret_code = RC::kOk;
    rpc_context->inner.buffer_addr = iobuf;
