...
namespace patronus
{
inline const char *wc_opcode_str(ibv_wc_opcode opcode)
{
    switch (opcode)
    {
    case IBV_WC_SEND:
...
// inline std::ostream &operator<<(std::ostream &os, const ibv_wc_status
// &status)
// {
//     os << ibv_wc_status_str(status) << "(" << (int) status << ")";
//     return os;
// }
// inline std::ostream &operator<<(std::ostream &os, const ibv_wc_opcode
// &opcode)
// {
...
//        << ", opcode: " << (int) wr.opcode << ", send_flags: " <<
//        wr.send_flags
