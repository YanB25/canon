...
public:
    ...
    RetCode commit(AsyncContext<RDMA> *rw_ctx,
                   CoroContext *ctx,
                   util::TraceView trace = util::nulltrace)
    {
        ...
        WRID signal_wrid(rw_ctx, WRID_WAIT_RDMA, 1);
        WRID nonsignal_wrid(rw_ctx, WRID_WAIT_RDMA, 0);
        ...
        for (size_t i = 0; i < idx_; ++i)
        {
            ...
            if (last)
            {
                ...
                send_wrs_[i].wr_id = signal_wrid.val();
            }
            else
            {
                send_wrs_[i].wr_id = nonsignal_wrid.val();
            }
        }
        DVLOG(V) << "[patronus][batch] commiting " << idx_
                 << " WR with wr_id: " << signal_wrid;
        ...
        ctx->record_yield_reason(rw_ctx, signal_wrid);
