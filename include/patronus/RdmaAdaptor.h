...
struct RdmaTraceRecord
{
    ...
    void trace_pin(const std::string_view name_view)
    {
        trace_timer.pin(std::string(name_view));
    }
};
...
public:
    ...
    ~RdmaAdaptor()
    {
        if constexpr (debug())
        {
            if (!ongoing_remote_handle_->empty())
            {
                const auto &handle = *ongoing_remote_handle_->begin();
                LOG(WARNING)
                    << "[rdma-adpt] possible handle leak for " << handle
                    << ". known leak nr: " << ongoing_remote_handle_->size()
                                 ...
                                 flag_t flag) override
    {
        ...
        {
            return alloc_handle(lease_to_exposed_raddr(lease), size, lease);
        }
        else
        {
            ...
            // }
            auto ret = RemoteMemHandle{GlobalAddress::Null(), 0, lease.ec()};
            ret.set_invalid();
            CHECK(false) << "** Failed to acquire_perm: " << lease.ec()
                         << ". Detail: gaddr: " << gaddr << ", size: " << size
                         << ", hint: " << alloc_hint
    ...
    GlobalAddress lease_to_exposed_raddr(const Lease &lease) const
    {
        CHECK(lease.success());
    ...
    GlobalAddress remote_alloc(size_t size, hint_t hint) override
    {
        ...
        {
            auto ret =
                GlobalAddress(0, (uint64_t) it->second->alloc(size, coro_ctx_));
            return ret;
        }
                      ...
                      RemoteMemHandle &handle) override
    {
        CHECK(handle.valid());
        auto &lease = *(Lease *) handle.private_data();
        CHECK_GE(raddr.offset, handle.raddr().offset);
        auto offset = raddr.offset - handle.raddr().offset;
        ...
        if (unlikely(ec == kNoMem))
        {
            ...
            DCHECK(batch_.empty());
            return rdma_read(rdma_buf, raddr, size, flag, handle);
        }
                       ...
                       RemoteMemHandle &handle) override
    {
        ...
        if (unlikely(ec == kNoMem))
        {
            CHECK_EQ(patronus_->commit(batch_, coro_ctx_), kOk);
            DCHECK(batch_.empty());
            return rdma_write(raddr, rdma_buf, size, flag, handle);
        }
                     ...
                     RemoteMemHandle &handle) override
    {
        ...
        if (unlikely(rc == kNoMem))
        {
            CHECK_EQ(patronus_->commit(batch_, coro_ctx_), kOk);
            DCHECK(batch_.empty());
            return rdma_faa(raddr, value, rdma_buf, flag, handle);
        }
        return rc;
    }
                     ...
                     RemoteMemHandle &handle) override
    {
        ...
        auto gaddr = vaddr_to_gaddr(vaddr);
        auto &lease = *(Lease *) handle.private_data();
        auto handle_gaddr = handle.raddr();
        CHECK_GE(gaddr.offset, handle_gaddr.offset);
        auto offset = gaddr.offset - handle_gaddr.offset;
        flag |= (flag_t) RWFlag::kNoLocalExpireCheck;
        if (bypass_prot_)
        {
            flag |= (flag_t) RWFlag::kUseUniversalRkey;
        ...
        if (unlikely(rc == kNoMem))
        {
            CHECK_EQ(patronus_->commit(batch_, coro_ctx_), kOk);
            DCHECK(batch_.empty());
            return rdma_cas(gaddr, expect, desired, rdma_buf, flag, handle);
        }
        return rc;
    }
    RetCode commit(util::TraceView = util::nulltrace) override
    {
        if constexpr (::config::kEnableRdmaTrace)
        {
            if (trace_enabled())
    ...
    {
        auto gaddr = patronus_->to_exposed_gaddr(addr);
        return gaddr;
    }
    ...
    void reg_overwrite_allocator(uint64_t hint,
                                 IAllocator::pointer allocator) override
    {
...
private:
    ...
    std::unordered_map<uint64_t, IAllocator::pointer> overwrite_allocators_;

    PatronusBatchContext<define::kMaxCoroNr> batch_;
    ...
    void free_handle(RemoteMemHandle &handle)
    {
        ...
        if constexpr (debug())
        {
            auto ret = ongoing_remote_handle_->erase(handle.view());
            CHECK_EQ(ret, 1) << "** freeing an not-allocated-handled.";
        }

        handle.set_invalid();
    }
    RemoteMemHandle alloc_handle(GlobalAddress raddr, size_t size, Lease &lease)
    {
        bool success = lease.success();
        DCHECK_EQ(raddr.nodeID, 0) << "** raddr.nodeID here should be zero, "
                                      "because it is client-visible";
        RemoteMemHandle handle(raddr, size, lease.ec());
        auto stored_lease = std::make_unique<Lease>(std::move(lease));
        handle.set_private_data(stored_lease.release());
        if constexpr (debug())
        {
            CHECK(ongoing_remote_handle_->insert(handle.view()).second);
        }
        CHECK(handle.valid());
        if (unlikely(!success))
        {
