...
public:
    ...
    {
        loop_ = loop;
        clock_gettime(CLOCK_REALTIME, &e_);
        uint64_t ns_all =
            (e_.tv_sec - s_.tv_sec) * 1000000000ull + (e_.tv_nsec - s_.tv_nsec);
        ns_ = ns_all / loop;
...
std::ostream &operator<<(std::ostream &os, const ContTimer<true> &t)
{
    ...
    os << fmt::format("[{}]: *summary* takes {} ns ({} ms)",
                      t.name_,
                      total_ns,
                      total_ns / 1e6)
       << std::endl;
...
{
public:
    ChronoTimer(bool enabled = true) : enabled_(enabled)
    {
        if (enabled_)
        {
