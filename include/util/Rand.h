...
{
    return TLS<std::mt19937>((std::hash<std::string>{}(FLAGS_node_id) << 16) +
                             util::get_thread_id());
}
...
{
    return TLS<std::ranlux48_base>(util::get_thread_id());
}
...
inline char fast_pseudo_rand_printable_char()
{
    ...
    return alphanum[fast_pseudo_rand_int(0, size_of_alpha - 1)];
}
...
inline void fast_pseudo_fill_buf(char *s, size_t len)
{
    ...
    {
        s[i] = fast_pseudo_rand_printable_char();
    }
}
