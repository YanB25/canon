...
struct RemoteMemHandleView
{
    ...
    {
        return view.raddr == raddr && view.size == size &&
...
{
    os << "{RemoteMemHandleView raddr: " << view.raddr
...
{
public:
    RemoteMemHandle(GlobalAddress raddr,
                    size_t size,
                    patronus::AcquireRequestStatus ec)
        : raddr_(raddr), size_(size), valid_(true), ec_(ec)
    {
    }
    RemoteMemHandle()
        : raddr_(GlobalAddress::Null()),
          size_(0),
          valid_(false),
    ...
    RemoteMemHandleView view() const
    {
        ...
        return RemoteMemHandleView{
            .raddr = raddr_, .size = size_, .private_data = private_data_};
    }
    ...
    {
        return raddr_ == rhs.raddr_ && size_ == rhs.size_ &&
               private_data_ == rhs.private_data_;
    }
...
{
    os << "{RemoteMemHandle: raddr: " << handle.raddr()
...
public:
    ...
    [[nodiscard]] virtual RetCode rdma_faa(GlobalAddress gaddr,
                                           ...
                                           flag_t flag,  // RWFlag
                                           RemoteMemHandle &) = 0;
    virtual void debug_explain_raddr(GlobalAddress raddr)
    {
        LOG(INFO) << "(unknown how to explain) " << raddr;
    }
    [[nodiscard]] virtual RetCode commit(util::TraceView = util::nulltrace) = 0;
     ...
     * @param hint
     */
    virtual void reg_overwrite_allocator(uint64_t hint,
                                         ::mem::IAllocator::pointer) = 0;

    // called by server side
    virtual GlobalAddress to_exposed_raddr(void *addr) = 0;
    virtual void *from_exposed_raddr(GlobalAddress gaddr) = 0;
    ...
    virtual std::shared_ptr<avis::PTL> ptl()
    {
        return nullptr;
    }
};
