        ...
        monitor_thread = std::thread(
            [this, duration, report]()
            {
                while (likely(!is_finish()))
                {
                    ...
                    double ns_per_op_per_thread =
                        ns_per_op / bench_threads_.size();

                    // determine finished
                    bool should_finish = true;
                    for (size_t i = 0; i < bench_threads_.size(); ++i)
                    {
                        if (!thread_finished_[i].load(
...
public:
    ...
    void reset(T min, T max, T range)
    {
        min_ = min;
        max_ = max;
        range_ = range;
        ...
        collect_min_ = std::numeric_limits<T>::max();
        collect_max_ = std::numeric_limits<T>::lowest();
        less_than_min_ = 0;
        larger_than_max_ = 0;
        collected_nr_ = 0;
    ...
    OnePassBucketMonitor(const OnePassBucketMonitor<T> &rhs) = default;
    ...
    // query of percentile may fail: lose accuracy
    std::optional<T> percentile(double p) const
    {
        CHECK(!as_distribution_);
    ...
    T min_as_distribution() const
    {
        CHECK(as_distribution_);
        return buckets_.back();
    }

    T max_as_distribution() const
    {
        CHECK(as_distribution_);
        return buckets_.front();
    }

    // [index, value] of percentile
    std::pair<size_t, T> percentile_as_distribution(double p) const
    {
        CHECK(as_distribution_);
        size_t index = p * buckets_.size();
        std::min(index, buckets_.size());

        return {index, buckets_[index]};
    }

    // [not_zero_nr, not_zero_ratio]
    std::pair<size_t, double> not_zero_as_distribution() const
    {
        CHECK(as_distribution_);
        size_t not_zero_nr = std::count_if(
            buckets_.begin(), buckets_.end(), [](T t) { return t != T{}; });
        return {not_zero_nr, 1.0 * not_zero_nr / buckets_.size()};
    }
    ...
    void merge(const OnePassBucketMonitor<T> &rhs)
    {
        CHECK_EQ(min_, rhs.min_);
        CHECK_EQ(max_, rhs.max_);
        CHECK_EQ(range_, rhs.range_);
        CHECK_EQ(range_nr_, rhs.range_nr_);
        CHECK_EQ(buckets_.size(), rhs.buckets_.size());

        collect_min_ = std::min(collect_min_, rhs.collect_min_);
        collect_max_ = std::max(collect_max_, rhs.collect_max_);
        less_than_min_ += rhs.less_than_min_;
        larger_than_max_ += rhs.larger_than_max_;
        collected_nr_ += rhs.collected_nr_;
        for (size_t i = 0; i < buckets_.size(); ++i)
        {
    ...
    OnePassBucketMonitor<T> generate_empty_one_same_type() const
    {
        return OnePassBucketMonitor<T>(min_, max_, range_);
    }

    void as_distribution()
    {
        std::sort(buckets_.begin(), buckets_.end(), std::greater<>());
        CHECK_EQ(less_than_min_, 0);
        CHECK_EQ(larger_than_max_, 0);
        as_distribution_ = true;
    }
                                ...
                                const OnePassBucketMonitor<T> &m)
{
    ...
    {
        os << "{min: " << m.min();
        auto median = m.percentile(0.5);
        auto p60 = m.percentile(0.60);
        auto p70 = m.percentile(0.60);
        auto p80 = m.percentile(0.60);
        auto p90 = m.percentile(0.90);
        auto p99 = m.percentile(0.99);
        auto p999 = m.percentile(0.999);
        auto p9999 = m.percentile(0.9999);
        if (median)
        {
            os << ", median: " << *median;
        }
        ...
        os << ", max: " << m.max() << ". collected " << m.data_nr() << " data";
        if (m.underflow_nr())
        {
            os << ", underflow: " << m.underflow_nr();
        }
        if (m.overflow_nr())
        {
            os << ", overflow: " << m.overflow_nr();
        }
        os << "}";
    }
    return os;
...
public:
    ...
    {
        ls_.push_back(t);
        while (unlikely(ls_.size() > limit_))
        {
            ls_.pop_front();
        }
    }
    ...
    T max() const
    {
        ...
        return *std::max_element(ls_.begin(), ls_.end());
    }
    T min() const
    {
        if (empty())
        ...
        return *std::min_element(ls_.begin(), ls_.end());
    }
    T diff() const
    {
        if (empty())
    ...
    T sum() const
    {
        return std::accumulate(ls_.begin(), ls_.end(), T{});
    }
    T avg() const
    {
        if (ls_.empty())
...
inline std::ostream &operator<<(std::ostream &os, const SequenceLimit<T> &s)
{
    os << std::endl;
    size_t idx = 0;
    for (const auto &item : s.ls_)
    {
        os << idx << ", " << item << std::endl;
        idx++;
    }
