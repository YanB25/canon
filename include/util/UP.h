...
template <typename T>
concept ConstOstreamable = requires(const T &t)
{
    std::cout << t;
};

template <typename T>
concept MutOstreamable = requires(T &t)
{
    std::cout << t;
};

template <typename T>
concept Ostreamable = ConstOstreamable<T> || MutOstreamable<T>;
...
// Fix for special type T, where overloads
// operator<<(..., T&)
// note the mutable reference above.
template <typename T>
inline std::ostream &operator<<(std::ostream &os, const pre_impl<T> &t) requires
    ConstOstreamable<T>
{
    os << (const T &) t.inner();
    return os;
...
inline std::ostream &operator<<(
    std::ostream &os,
    const pre_impl<T> &t) requires(!ConstOstreamable<T> && MutOstreamable<T>)
{
    os << (T &) t.inner();
    return os;
}
...
inline std::ostream &operator<<(std::ostream &os,
                                const pre_impl<std::chrono::nanoseconds> &ns)
{
    uint64_t num = ns.inner().count();
    if (num < 1000ull)
    {
        os << num << " ns";
    }
    else if (num < 1000ull * 1000)
    {
        os << 1.0 * num / 1000ull << " us";
    }
    else if (num < 1000ull * 1000ull * 1000ull)
    {
        os << 1.0 * num / (1000ull * 1000) << " ms";
    }
    else
    {
        os << 1.0 * num / (1000ull * 1000 * 1000) << " s";
    }
    return os;
}
inline std::ostream &operator<<(std::ostream &os,
                                const pre_impl<std::chrono::microseconds> &us)
{
    os << std::chrono::nanoseconds(us.inner());
    return os;
}
inline std::ostream &operator<<(std::ostream &os,
                                const pre_impl<std::chrono::milliseconds> &ms)
{
    os << std::chrono::nanoseconds(ms.inner());
    return os;
}
inline std::ostream &operator<<(std::ostream &os,
                                const pre_impl<std::chrono::seconds> &s)
{
    os << std::chrono::nanoseconds(s.inner());
    return os;
}
inline std::ostream &operator<<(std::ostream &os,
                                const pre_impl<std::chrono::minutes> &mi)
{
    os << std::chrono::nanoseconds(mi.inner());
    return os;
}
inline std::ostream &operator<<(std::ostream &os,
                                const pre_impl<std::chrono::hours> &h)
{
    os << std::chrono::nanoseconds(h.inner());
    return os;
}
inline std::ostream &operator<<(std::ostream &os,
                                const pre_impl<std::chrono::days> &d)
{
    os << d.inner().count() << " d";
    return os;
}
                                ...
                                const pre_impl<std::shared_ptr<T>> &p)
{
    ...
    {
        os << "shared_ptr(" << (void *) p.inner().get() << ")";
    }
    return os;
}
template <typename T>
...
template <typename T>
inline std::ostream &operator<<(std::ostream &os,
                                const pre_impl<std::weak_ptr<T>> &p)
{
    auto upgrade = p.inner().lock();
    if constexpr (ostreamable_v<T>)
    {
        if (upgrade)
        {
                                ...
                                const pre<std::optional<T>> &t)
{
    ...
    {
        os << "optional(" << util::pre(t.inner().value(), t.ctx()) << ")";
    }
    else
    {
        os << "nullopt";
...
// also, this is the fallback way
template <typename T>
inline std::ostream &operator<<(std::ostream &os,
                                const pre_impl<T> &t) requires(!Ostreamable<T>)
{
...
template <typename T, typename... Ts>
struct fmt::formatter<util::pre_with_names<T, Ts...>>
{
    // Parses format specifications of the form ['f' | 'e'].
    constexpr auto parse(format_parse_context &ctx) -> decltype(ctx.begin())
    {
    ...
    template <typename FormatContext>
    auto format(const util::pre_with_names<T, Ts...> &p,
