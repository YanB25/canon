...
void test_list_umr(DSM::pointer dsm)
{
    ...
    for (size_t i = 0; i < FLAGS_mr_nr; ++i)
    {
        ...
        CHECK_EQ((uint64_t) mr->addr % 64, 0);
        mrs.push_back(mr);
    }
    umr_ctx.qp = qp;
    umr_ctx.cq = cq;
...

    {
        ...
        // dsm->prepare_cas(dsm->get_node_id(),
        //                  rkey,
        //                  remote_addr,
        //                  size,
        //                  (uint64_t) compare.data(),
        //                  (uint64_t) mask.data(),
        //                  (uint64_t) rdma_buf.buffer,
        //                  (uint64_t) mask.data(),
        //                  rdma_buf.buffer,
        //                  nullptr);
        // dsm->prepare_write(rdma_buf.buffer,
        //                    dsm->get_node_id(),
        //                    rkey,
        //                    remote_addr,
        //                    size,
        //                    nullptr);
        std::vector<char> add_val(32, 0);
        std::vector<char> boundary(32, 0xff);
        dsm->prepare_faa(dsm->get_node_id(),
                         rkey,
                         remote_addr,
                         size,
    ...
    {
        LOG(INFO) << "After CAS: tell you the situation: ";
        for (auto *mr : mrs)
        {
            dump(mr);
        }
