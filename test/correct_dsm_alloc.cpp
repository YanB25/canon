...
struct ThreadContext
{
    ...
    std::shared_ptr<memory::DSMAllocator> alloc;
    OnePassBucketMonitor<uint64_t> bucket{0, FLAGS_machine_nr, 1};
};
...
public:
                   ...
                   bool) override
    {
        ...
        {
            auto gaddr = tlc.alloc->alloc(alloc_size);
    ...
    {
        auto &tlc = bls.thread();
        tlc.alloc = std::make_shared<memory::DSMAllocator>(dsm_.get());
    ...
    void on_thread_end_bench(BLS &bls, const Config &conf) override
    {
        ...
        tlc.bucket.as_distribution();
        CHECK_LE(
            tlc.bucket.max_as_distribution() - tlc.bucket.min_as_distribution(),
            1)
            << "** distribution to nodes not uniform: "
            << util::pre_distribution(tlc.bucket);
    ...
    {
        LOG(INFO) << "Begin validation... (this may take long)";
...
int main(int argc, char *argv[])
{
    ...
    f.add_option<size_t>("alloc_nr", {1_K});
    f.add_option<size_t>("alloc_size", {1_KB});
