...
int main(int argc, char *argv[])
{
    ...
    {
        std::string hei;
        LOG(INFO) << PRE(sizeof(std::string_view(hei)));
    }
    {
        uint64_t arr[4]{};
        // auto mut = util::BitsViewMut(arr, sizeof(arr));
        // LOG(INFO) << mut.hex_dump();
        // LOG(INFO) << mut.as_const().hex_dump();
        auto b1 = util::Bits(arr, sizeof(arr));
        auto b2 = util::Bits(arr, sizeof(arr));
        LOG(INFO) << (b1 & b2);
        LOG(INFO) << b1.view().test(10);
    }
