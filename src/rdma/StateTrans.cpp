...
{
    return modifyQPtoInit(qp, context->port);
}
                   ...
                   int gid_index)
{
    ...
    fillAhAttr(&attr.ah_attr, remoteLid, remoteGid, port, gid_index);
    ...
    // flags |= IBV_EXP_QP_OOO_RW_DATA_PLACEMENT;
...
bool modifyQPtoRTR(struct ibv_qp *qp,
                   ...
                   RdmaContext *context)
{
    return modifyQPtoRTR(
        qp, remoteQPN, remoteLid, remoteGid, context->port, context->gidIndex);
}
