                   ...
                   std::optional<ibv_exp_msg_model> msg_model)
{
    ...
    ibv_exp_res_domain *res_doms[kCorePerNuma] = {};
    ...
    for (int i = 0; i < devicesNum; ++i)
    {
        ...
        if (std::string(device_name) == rnic)
        {
            devIndex = i;
            found_device = true;
            VLOG(V) << "[system] Select RNIC " << device_name << " at devIndex "
                    << i;
            break;
        }
    }

    CHECK(found_device) << "** Not RNIC named `" << rnic
                        << "` found. Device list is "
                        << util::pre(device_names);
    ...
    else
    {
        ...
        {
            // LOG_FIRST_N(WARNING, 1) << "TODO: Although device seems to
            // support "
    ...
    if (thread_model.has_value() || msg_model.has_value())
    {
        ...
        for (size_t i = 0; i < kCorePerNuma; ++i)
        {
...
CreateResourcesExit:
    ...
    auto size = sizeof(res_doms) / sizeof(res_doms[0]);
...
{
    bool rc = true;
    auto size = sizeof(context->res_doms) / sizeof(context->res_doms[0]);
    for (size_t i = 0; i < size; ++i)
    {
...
ibv_mr *createMemoryRegion(uint64_t mm,
                           ...
                           uint64_t extra_exp_access_flags)
{
    ...
    in.exp_access |= extra_exp_access_flags;

    auto *mr = ibv_exp_reg_mr(&in);
    if (unlikely(mr == nullptr))
    {
        PLOG(ERROR) << "** Failed to ibv_exp_reg_mr at " << (void *) mm
                    << ", size " << mmSize;
    }
...
ibv_mr *createMemoryRegionOnChip(uint64_t mm, uint64_t mmSize, RdmaContext *ctx)
{
    ...
    if (!dm)
    {
        PLOG(ERROR) << "Allocate on-chip memory failed";
        return nullptr;
    }
    ctx->dm = dm;
...
ibv_mr *doCreateMemoryRegionOnChip(uint64_t mm,
                                   uint64_t size,
                                   RdmaContext *ctx,
                                   size_t retry_limit)
{
    size_t try_nr = 0;
    while (try_nr <= retry_limit)
    {
        ibv_mr *ret = createMemoryRegionOnChip(mm, size, ctx);
        if (ret)
        {
            return ret;
        }
        PLOG(ERROR)
            << "Failed to allocate memory region on device memory for size "
            << size << ". Sleep for a while.";
        std::this_thread::sleep_for(1s);
        try_nr++;
    }
    LOG(FATAL) << "Failed to allocate on device memory. ";
    return nullptr;
}
...
ibv_qp *createQueuePair(ibv_pd *pd,
                        ibv_context *ctx,
                        ibv_qp_type mode,
                        ibv_cq *send_cq,
                        ibv_cq *recv_cq,
                        ...
                        uint32_t create_flags)
{
    ...
    attr.exp_create_flags |= create_flags;
    if (create_flags & IBV_EXP_QP_CREATE_UMR)
    {
        attr.comp_mask |= IBV_EXP_QP_INIT_ATTR_MAX_INL_KLMS;
        attr.max_inl_send_klms = MAX_SEND_WQE_INLINE_KLMS;
    }

    auto *qp = ibv_exp_create_qp(ctx, &attr);
    PLOG_IF(ERROR, qp == nullptr) << "Failed to create qp.";
    return qp;
}
...
{
    return fillAhAttr(
        attr, remoteLid, remoteGid, context->port, context->gidIndex);
}
