               ...
               const WcHandler &handler)
{
    ...
    do
    {
        ...
        CHECK_GE(new_count, 0) << "PollCQ failed.";
        CHECK_LE(new_count, 1) << "** impossible value";
        if (new_count)
        {
...
int pollOnce(ibv_cq *cq, int pollNumber, struct ibv_wc *wc)
{
    ...
    {
        PLOG(ERROR) << "[QP] operation failed: wc: " << *wc;
        return -1;
    }
    else
    {
              ...
              std::optional<uint32_t> imm)
{
    ...
    constexpr static size_t V = ::config::verbose::kRdmaOperation;
    VLOG(V) << "[operation] RDMA send: source: " << (void *) source
            << ", size: " << size << ", remote_qpn: " << remoteQPN
            << ", signal: " << isSignaled << ", inline: " << isInlined
            << ", wr_id: " << wr_id;

    if (ibv_post_send(qp, &wr, &wrBad))
    {
        PLOG(ERROR) << "Send with RDMA_SEND failed.";
        return false;
    }
    return true;
}
              ...
              uint64_t wrID)
{
    ...
    DVLOG(::config::verbose::kRdmaOperation) << "[READ] " << wr;
               ...
               uint64_t wrID)
{
    ...
    DVLOG(::config::verbose::kRdmaOperation) << wr;

    if (ibv_post_send(qp, &wr, &wrBad) != 0)
    {
        PLOG(FATAL) << "ibv_post_send failed: " << *wrBad;
        // sleep(10);
        return false;
    }
    return true;
...
constexpr static uint32_t kMagic = 0b1010101010;
constexpr static uint16_t kMask = 0b1111111111;
...
void rdmaQueryDevice()
{
    ...
    exp_attr.comp_mask =
        ...
        IBV_EXP_DEVICE_ATTR_WITH_HCA_CORE_CLOCK | IBV_EXP_DEVICE_ATTR_UMR |
    ...
    printf("max_lkm_list_size: %u\n", exp_attr.umr_caps.max_klm_list_size);
    printf("max_send_wqe_inline_klms: %u\n",
           exp_attr.umr_caps.max_send_wqe_inline_klms);
    printf("max_umr_recursion_depth: %u\n",
           exp_attr.umr_caps.max_umr_recursion_depth);
    printf("max_umr_stride_dimension: %u\n",
           exp_attr.umr_caps.max_umr_stride_dimension);
    // printf("IBV_EXP_DEVICE_SHARED_MR: %d\n", )
    // printf("IBV_EXP_FLAGS unknown 1: %d, 2: %d, 3: %d\n",
    //     exp_attr.exp_device_cap_flags & )
    const auto &ooo = exp_attr.ooo_caps;
    LOG(INFO) << "OOO: "
              << PRE(ooo.rc_caps, ooo.xrc_caps, ooo.dc_caps, ooo.ud_caps);
