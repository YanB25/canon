...
DSMKeeper::DSMKeeper(std::vector<std::unique_ptr<ThreadConnection>> &thCon,
                     ...
                     UnreliableConnection<kCorePerNuma> &umsg,
...
void DSMKeeper::initLocalMeta()
{
    ...
    for (size_t i = 0; i < kCorePerNuma; ++i)
    {
        ...
        exchangeMeta.umsgs[i].lid = umsg.context(i).lid;
        memcpy((char *) exchangeMeta.umsgs[i].gid,
               (char *) (&umsg.context(i).gid),
               16 * sizeof(uint8_t));
    }
}
                                          ...
                                          size_t remoteID)
{
    ...
    {
        meta.dirRcQpn2app[dirID][k] = dir.QPs[k][remoteID].ibqp()->qp_num;
    }
...
void DSMKeeper::setExchangeMeta(uint16_t remoteID)
{
    for (int i = 0; i < NR_DIRECTORY; ++i)
    {
        ...
        for (int k = 0; k < kMaxAppThread; ++k)
        {
            exchangeMeta.dirRcQpn2app[i][k] =
                c->QPs[k][remoteID].ibqp()->qp_num;
        }
    }
    ...
    for (int i = 0; i < kMaxAppThread; ++i)
    {
        ...
        for (int k = 0; k < NR_DIRECTORY; ++k)
        {
            ...
            exchangeMeta.appRcQpn2dir[i][k] =
                c->QPs[k][remoteID].ibqp()->qp_num;
        }
    }
}

void DSMKeeper::connectUnreliableMsg(UnreliableConnection<kCorePerNuma> &, int)
{
}
...
{
    auto *qp = th.QPs[dirID][remoteID].ibqp();
...
void DSMKeeper::connectDir(DirectoryConnection &dir,
                           ...
                           const ExchangeMeta &exMeta)
{
    auto *qp = dir.QPs[appID][remoteID].ibqp();
...
void DSMKeeper::applyExchangeMeta(uint16_t remoteID, const ExchangeMeta &exMeta)
{
    ...
    // umsg
    for (size_t to = 0; to < kCorePerNuma; ++to)
    {
        for (size_t from = 0; from < kCorePerNuma; ++from)
        {
            ...
            fillAhAttr(&ah_attr,
                       ...
                       &umsg.context(from));
            auto *ah =
                ibv_create_ah(CHECK_NOTNULL(umsg.context(from).pd), &ah_attr);
            if (unlikely(ah == nullptr))
            {
                LOG(FATAL) << "failed in ibv_create_ah. umsg.context.pd: "
                           << (void *) umsg.context(from).pd
                           << ", errno: " << errno;
            }
...
std::optional<uint64_t> DSMKeeper::try_sum(const std::string &sum_key,
                                           uint64_t value,
                                           std::chrono::nanoseconds timeout)
{
    ...
    {
        key = key_prefix + std::to_string(i);
        char *buf_ret = try_get(key.c_str(), key.size(), nullptr, timeout);
        if (buf_ret)
        {
            ret += *(uint64_t *) buf_ret;
            free(buf_ret);
