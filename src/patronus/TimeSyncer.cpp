...
void TimeSyncer::signal_finish()
{
    for (size_t nid = 0; nid < dsm_->getClusterSize(); ++nid)
    {
        ...
        auto rdma_buffer = dsm_->get_rdma_buffer(sizeof(SyncFinishedMessage) *
                                                 dsm_->getClusterSize());
        ...
        dsm_->unreliable_send((char *) &sync_finish_message,
                              ...
                              nid,
                              kDirId);
        dsm_->put_rdma_buffer(std::move(rdma_buffer));
    }
}

void TimeSyncer::do_sync()
{
    auto rdma_buffer_ = dsm_->get_rdma_buffer(sizeof(ClockInfo));
    ...
    while (true)
    {
        ...
        {
            DVLOG(V) << "[TimeSyncer] magic == 0. node " << target_gaddr_.nodeID
                     << " not ready. ";
        ...
        auto after_client_time =
            to_ns(now) + clock_info_.adjustment.load(std::memory_order_relaxed);
        DVLOG(V) << "[TimeSyncer] adjust " << this_adjustment
