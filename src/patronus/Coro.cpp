...
bool ServerCoroBatchExecutionContext::commit(AsyncContext<RDMA> *rw_ctx,
                                             WRID &o_wrid)
{
    ...
    {
        ret = commit_with_mw_reuse_optimization(rw_ctx, o_wrid);
    }
    else
    {
        ret = commit_wo_mw_reuse_optimization(rw_ctx, o_wrid);
    }
...
bool ServerCoroBatchExecutionContext::commit_wo_mw_reuse_optimization(
    AsyncContext<RDMA> *rw_ctx, WRID &o_wrid)
{
    WRID signal_wrid(rw_ctx, WRID_WAIT_MW, 1);
    WRID nonsignal_wrid(rw_ctx, WRID_WAIT_MW, 0);
    o_wrid = signal_wrid;
    ...
    if (unlikely(ret != 0))
    {
        CHECK(false) << "[patronus] failed to ibv_post_send for bind_mw. "
                        "failed wr : "
                     << WRID(bad_wr_->wr_id) << ", signal_wrid: " << signal_wrid
    ...
    DVLOG(V) << "[patronus] batch commit with wr_id: " << signal_wrid
