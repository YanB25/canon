...
thread_local std::unique_ptr<::mem::SlabAllocator>
    Patronus::rdma_client_buffer_;
thread_local std::array<std::unique_ptr<mem::MWPool>, NR_DIRECTORY>
    Patronus::mw_pool_;
...
thread_local Patronus::PatronusBatchContextT Patronus::one_shot_batch_ctx_{};
thread_local ssize_t Patronus::ongoing_rdma_nr_{0};
thread_local ssize_t Patronus::ongoing_rpc_nr_{0};
...
Patronus::Patronus(const PatronusConfig &conf) : conf_(conf)
{
    ...
    dsm_config.numa_id = conf.numa_id;
    dsm_config.rnic = conf.rnic;
                               ...
                               CoroContext *ctx)
{
    ...
    auto rdma_buf = get_rdma_message_buffer(sizeof(AcquireRequest));
    DCHECK_GE(rdma_buf.size, sizeof(AcquireRequest));
    auto *rpc_context = get_rpc_context(ctx);

    Lease ret_lease;
    ret_lease.node_id_ = node_id;
    ret_lease.no_gc_ = no_gc;

    auto *msg = (AcquireRequest *) rdma_buf.buffer;
    msg->type = type;
    msg->cid.node_id = get_node_id();
    msg->cid.thread_id = tid;
    msg->cid.coro_id = ctx ? ctx->coro_id() : kNotACoro;
    msg->cid.async_context = (uint64_t) rpc_context;
    msg->dir_id = dir_id;
    msg->key = key_or_hint;
    msg->size = size;
    msg->required_ns = ns;
    ...
    rpc_context->inner.ret_lease = &ret_lease;
    rpc_context->inner.dir_id = dir_id;
    rpc_context->inner.request = (BaseMessage *) msg;
    rpc_context->inner.ret_code = RC::kOk;
    ...
    DVLOG(V) << "[patronus] get_lease from node_id: " << node_id
             ...
             << ". Message: " << *(BaseMessage *) rdma_buf.buffer
             << ". coro: " << pre_coro_ctx(ctx);

    dsm_->unreliable_send(
        rdma_buf.buffer, sizeof(AcquireRequest), node_id, dir_id);
    ...
    else
    {
        ...
        ctx->record_yield_reason(rpc_context, (BaseMessage *) msg);
        ctx->yield_to_master();
    ...
    if (!ret_lease.success())
    {
        CHECK_NE(ret_lease.status_, AcquireRequestStatus::kReserved)
            << "** getting ret_lease is not succeeded, but the status is "
               "reserved. Lease: "
            << ret_lease;
        DCHECK_NE(rpc_context->inner.ret_code, RC::kOk);
    }
                                    ...
                                    CoroContext *ctx)
{
    ...
    auto rdma_buf = get_rdma_message_buffer(sizeof(LeaseModifyRequest));
    DCHECK_GE(rdma_buf.size, sizeof(LeaseModifyRequest));
    auto *rpc_context = get_rpc_context(ctx);

    auto *msg = (LeaseModifyRequest *) rdma_buf.buffer;
    msg->type = type;
    msg->cid.node_id = get_node_id();
    msg->cid.thread_id = tid;
    msg->cid.coro_id = ctx ? ctx->coro_id() : kNotACoro;
    ...
    dsm_->unreliable_send(
        rdma_buf.buffer, sizeof(LeaseModifyRequest), target_node_id, dir_id);
    ...
    auto ret = rpc_context->inner.ret_code;
...
size_t Patronus::handle_response_messages(msg_desc_t *msg_descs,
                                          size_t msg_nr,
                                          coro_t *coro_buf)
{
    size_t cur_idx = 0;
    for (size_t i = 0; i < msg_nr; ++i)
    {
        ...
        DVLOG(V) << "[patronus] handling response msg: " << (*base);
        auto response_type = base->type;
        auto coro_id = base->cid.coro_id;

        // some RPC is not async, i.e., the AdminReq from patronus->finished()
        // In this case, async_context is (uint64_t) nullptr.
        if (base->cid.async_context != 0)
        {
            auto *rpc_context = (AsyncContext<RPC> *) base->cid.async_context;
            DCHECK_EQ(rpc_context->coro_id, base->cid.coro_id)
                << "dump: " << rpc_context << ", msg: " << *base;
            DCHECK_EQ(rpc_context->tid, get_thread_id())
                << "dump: " << rpc_context << ", msg: " << *base;
            rpc_context->coro_state->match_yield_reason(rpc_context, base);
        }
...
{
    auto *rpc_context = (AsyncContext<RPC> *) resp->cid.async_context;

    auto *request = (AcquireRequest *) rpc_context->inner.request;
    ...
    auto &ret_lease = *rpc_context->inner.ret_lease;
    if (likely(resp->status == AcquireRequestStatus::kSuccess))
    {
        ...
        ret_lease.dir_id_ = rpc_context->inner.dir_id;
                                              ...
                                              CoroContext *ctx)
{
        ...
        req->type == RpcType::kAcquireWLeaseReq)
    {
        if (use_mr)
        {
            auto *rdma_ctx = DCHECK_NOTNULL(dsm_->get_dir_rdma_context(dirID));
            if (bind_buf)
            {
    ...
    std::map<std::pair<size_t, size_t>, TraceView> &recov)
{
    ...
    for (size_t i = 0; i < rdma_nr; ++i)
    {
        ...
        auto should_signal = wr_id.u8_l();
        auto prefix = wr_id.u8_h();
        auto *rw_context = wr_id.ptr();

        auto *coro_state = rw_context->coro_state;
        coro_state->match_yield_reason(rw_context, wr_id);

        DCHECK_EQ(prefix, WRID_WAIT_RDMA)
            << "Expecting " << pre_wrid_prefix(WRID_WAIT_RDMA) << ", got "
            << pre_wrid_prefix(prefix) << ". WRID: " << wr_id;

        auto node_id = rw_context->inner.target_node;
        auto dir_id = rw_context->inner.dir_id;
        auto coro_id = rw_context->coro_id;
        CHECK_NE(coro_id, kMasterCoro)
            << "** Coro master should not issue R/W.";

        rw_context->inner.trace_view.pin("arrived");

        if (likely(wc.status == IBV_WC_SUCCESS))
        {
            rw_context->inner.wc_status = IBV_WC_SUCCESS;
            DVLOG(V) << "[patronus] handle rdma finishes SUCCESS for coro "
                     << (int) coro_id << ". set "
                     << (void *) rw_context->inner.wc_status
                     << " to IBV_WC_SUCCESS";
        }
        else
        {
            LOG(WARNING) << "[patronus] rdma R/W/CAS failed. wr_id: " << wr_id
                         ...
                         << ", coro_id: " << (int) coro_id
                         << ". detail: " << wc;
            rw_context->inner.wc_status = wc.status;
            recov.emplace(std::make_pair(node_id, dir_id),
                          rw_context->inner.trace_view);
            fail_nr++;
        }
        rw_context->ready.store(true, std::memory_order_release);

        if (likely(should_signal))
        {
            coro_buf[cur_idx++] = coro_id;
        }
        else
        {
            LOG(WARNING) << "[patronus] Encounter unsignal RDMA WC. wr_id: "
                         << wr_id;
        }
    }
    return fail_nr;
...
void Patronus::finished(uint64_t key)
{
    ...
    msg->cid.async_context = (uint64_t) nullptr;
    msg->data = key;
    ...
    for (size_t i = 0; i < dsm_->getClusterSize(); ++i)
    {
        ...
        dsm_->unreliable_send(
            rdma_buf.buffer, sizeof(AdminRequest), i, to_dir_id);
    }
...
void Patronus::registerServerThread()
{
    ...
    if (unlikely(conf_.disable_mw))
    {
        // MW is diabled. Just allocate several
        alloc_mw_nr = 2 * define::kMaxCoroNr;
    }
    for (size_t dirID = 0; dirID < NR_DIRECTORY; ++dirID)
    {
        auto dsm = get_dsm();
        mw_pool_[dirID] = std::make_unique<mem::MWPool>(
...
{
    thread_local msg_desc_t msg_descs[config::umsg::kRecvLimit];
    ...
    size_t cur_idx = 0;
    cur_idx = handle_response_messages(msg_descs, nr, coro_buf);
    DCHECK_LT(cur_idx, limit);
    dsm_->return_buf_no_cpy(msg_descs, nr);

    size_t remain_nr = limit - nr;
    // should be enough
    constexpr static size_t kBufferSize = define::kMaxCoroNr + 1;
    static thread_local ibv_wc wc_buffer[kBufferSize];

    auto rdma_nr =
        dsm_->try_poll_rdma_cq(std::min(remain_nr, kBufferSize), wc_buffer);
    ...
    if constexpr (debug())
    {
        if (unlikely(fail_nr > 0))
        {
            DVLOG(SV) << "[patronus] handle rdma finishes got failure nr: "
                      << fail_nr << ". expect " << ongoing_rdma_nr();
        }
    }
    cur_idx += rdma_nr;
    DCHECK_LT(cur_idx, limit);
    ...
    if (unlikely(fail_nr))
    {
        ...
        if constexpr (debug())
        {
            DVLOG(SV) << "[patronus] failed. expect nr: " << ongoing_rdma_nr();
        }
        auto expect_rdma_nr = ongoing_rdma_nr();
        while (fail_nr < expect_rdma_nr)
        {
            auto another_nr = dsm_->try_poll_rdma_cq(kBufferSize, wc_buffer);
            LOG(WARNING)
        ...
        // forever.
        size_t got = msg_nr;
        size_t expect_rpc_nr = ongoing_rpc_nr();
        // TODO: maybe bug, I don't know, should I call ongoing_rpc_nr() each
        // time?
        while (got < expect_rpc_nr)
        {
            ...
            LOG_IF(WARNING, nr > 0)
                ...
                << expect_rpc_nr;
            nr = handle_response_messages(msg_descs, nr, coro_buf + cur_idx);
            dsm_->return_buf_no_cpy(msg_descs, nr);
            cur_idx += nr;
                                             ...
                                             CoroContext *ctx)
{
    ...
    if (resp->success)
    {
        ...
        {
            memcpy(rpc_context->inner.buffer_addr, resp->buffer, resp->size);
        }
...
void Patronus::server_serve(uint64_t key)
{
    ...
    server_master = CoroCall([this, key](CoroYield &yield)
                             { server_coro_master(yield, key); });
...
void Patronus::server_coro_master(CoroYield &yield, uint64_t wait_key)
{
    ...
    constexpr static size_t kMsgDescSize =
        ::config::umsg::kRecvLimit * sizeof(msg_desc_t);
    constexpr static size_t kServerBufferNr =
        define::kMaxCoroNr * MAX_MACHINE * kClientThreadPerServerThread;
                               ...
                               [](bool i) { return i; })))
    {
        ...
        if (msg_descs == nullptr)
        {
            msg_descs = (msg_desc_t *) CHECK_NOTNULL(msg_desc_pool.get());
        }
        size_t nr = dsm_->unreliable_try_recv_no_cpy(msg_descs,
        ...
        for (size_t i = 0; i < nr; ++i)
        {
            ...
            auto prefix = wrid.u8_h();
            auto should_signal = wrid.u8_l();
            auto *rw_ctx = wrid.ptr();
            DCHECK_EQ(prefix, WRID_WAIT_MW)
            ...
            if (unlikely(wc.status != IBV_WC_SUCCESS))
            {
                ...
                // need_recovery = true;
                rw_ctx->inner.wc_status = wc.status;
            }
            else
            {
                DVLOG(V) << "[patronus] server got dir CQE for coro "
                         ...
                         << mctx;
                rw_ctx->inner.wc_status = IBV_WC_SUCCESS;
            }
            rw_ctx->ready.store(true, std::memory_order_release);

            DVLOG(V) << "[patronus] server yield to coro " << (int) coro_id
                     << " for Dir CQE. rw_ctx: " << (void *) rw_ctx
                     << ", ready at " << (void *) &rw_ctx->ready
                     << ", rw_ctx at " << (void *) rw_ctx << ", ready is "
                     << rw_ctx->ready;

            auto *coro_state = rw_ctx->coro_state;
            coro_state->match_yield_reason(rw_ctx, wrid);

            mctx.yield_to_worker(coro_id);
        }

        // handle finished CQEs of RDMA
        // reuse wc_buffer
        nr = dsm_->try_poll_rdma_cq(kBufferSize, wc_buffer);
        for (size_t i = 0; i < nr; ++i)
        {
            auto &wc = wc_buffer[i];
            auto wr_id = WRID(wc.wr_id);
            ...
            auto should_signal = wr_id.u8_l();
            auto *rw_context = wr_id.ptr();
            CHECK_EQ(prefix, WRID_WAIT_OFFLOAD_MEMCPY)
                << "** unexpected prefix " << pre_wrid_prefix(prefix);

            auto node_id = rw_context->inner.target_node;
            DCHECK_EQ(node_id, get_node_id());
            DCHECK_EQ(rw_context->inner.dir_id, get_thread_id());
            auto coro_id = rw_context->coro_id;
            CHECK_NE(coro_id, kMasterCoro)
                << "** Coro master should not issue R/W.";
            ...
            rw_context->inner.trace_view.pin("arrived");
            rw_context->inner.wc_status = wc.status;
            CHECK_EQ(wc.status, IBV_WC_SUCCESS) << "** Expected succeeded, got "
                                                << ibv_wc_status_str(wc.status);
            rw_context->ready.store(true, std::memory_order_release);

            DVLOG(V) << "[patronus] server yield to coro " << (int) coro_id
                     << " for finished offloaded memcpy. rw_context: "
                     << (void *) rw_context << ", ready at "
                     << (void *) &rw_context->ready << ", rw_ctx at "
                     << (void *) rw_context << ", ready is "
                     << rw_context->ready;

            LOG(FATAL) << "TODO(reason): add yield reason";
            if (likely(should_signal))
            {
                auto *coro_state = rw_context->coro_state;
                coro_state->match_yield_reason(rw_context, wr_id);
            ...
            {
                LOG(WARNING) << "[patronus] encounter unsignaled WC. Possible "
                                "encounter error. wr_id: "
                             << wr_id;
            }
        }
    }
                                           ...
                                           CoroContext *ctx)
{
    ...
    auto resp_buf = get_rdma_message_buffer(sizeof(AcquireResponse));
    DCHECK_GE(resp_buf.size, sizeof(AcquireResponse));
    auto *resp_msg = (AcquireResponse *) resp_buf.buffer;
...
void Patronus::post_handle_request_admin(AdminRequest *req, CoroContext *ctx)
{
    ...
    auto resp_buf = get_rdma_message_buffer(sizeof(AdminResponse));
    DCHECK_GE(resp_buf.size, sizeof(AdminResponse));
    auto &resp_msg = *(AdminResponse *) resp_buf.buffer;
                                                ...
                                                CoroContext *ctx)
{
    ...
    auto resp_buf = get_rdma_message_buffer(sizeof(LeaseModifyResponse));
    DCHECK_GE(resp_buf.size, sizeof(LeaseModifyResponse));
    auto &resp_msg = *(LeaseModifyResponse *) resp_buf.buffer;
...
void Patronus::post_handle_request_memcpy(MemcpyRequest *req, CoroContext *ctx)
{
    ...
    if (req->offload)
    {
        constexpr static size_t kMaxOp = 64;
        static thread_local PatronusBatchContext<kMaxOp> offload_ctx;

        auto *rw_context = get_rw_context(ctx);
        rw_context->ready = false;
        rw_context->coro_id = ctx ? ctx->coro_id() : kNotACoro;
        rw_context->inner.wc_status = IBV_WC_SUCCESS;
        rw_context->inner.target_node = get_node_id();
        rw_context->inner.dir_id = get_thread_id();
        rw_context->inner.trace_view = util::nulltrace;
        // WRID wr_id(WRID_PREFIX_PATRONUS_MEMCPY_OFFLOAD, rw_ctx_id);
        // WRID wr_id((void *) rw_context);
        ...
        for (size_t i = 0; i < req->times; ++i)
        {
            ...
            // auto source_buffer = get_rdma_message_buffer(req->size);
            uint64_t dst_offset =
                req->size * fast_pseudo_rand_int(0, (max_size / req->size) - 1);
            char *source = start_addr + source_offset;
            // char *source = source_buffer.buffer;
            char *dst = start_addr + dst_offset;

            auto node_id = get_node_id();
            auto dir_id = get_thread_id();
            auto *qp = DCHECK_NOTNULL(dsm->get_th_qp(node_id, dir_id));
            auto lkey = dsm->get_dir_lkey(dir_id);
            // auto lkey = dsm->get_icon_lkey();
            auto rkey = dsm->get_rkey(node_id, dir_id);
            // LOG(INFO) << "[debug] memcpy from " << (void *) source << " to "
            //           << (void *) dst << ", size " << req->size << " with
            //           lkey "
            //           << lkey;
            do
            {
                auto ret = offload_ctx.prepare_write(qp,
                                                     node_id,
                                                     dir_id,
                                                     (uint64_t) dst,
                                                     (uint64_t) source,
                ...
                if (unlikely(ret == RC::kNoMem))
                {
                    offload_ctx.commit(rw_context, ctx).expect(RC::kOk);
                }
                else
                {
                    CHECK_EQ(ret, RC::kOk);
        ...
        auto ret = offload_ctx.commit(rw_context, ctx);
        CHECK(ret == RC::kOk || ret == RC::kNotFound)
            << "** unexpected RC on commit: " << ret;

        DCHECK(rw_context->ready);
        CHECK_EQ(rw_context->inner.wc_status, IBV_WC_SUCCESS);
                                  ...
                                  uint64_t wait_key)
{
           ...
           likely(!ddl_manager_.empty()))
    {
        ...
        msg_desc_t *start_msg_desc = nullptr;
        if (likely(!task_queue.empty()))
        {
            ...
            start_msg_desc = &(task->msg_descs[task->fetched_nr]);
        ...
        if (task)
        {
            prepare_handle_request_messages(
                DCHECK_NOTNULL(start_msg_desc), acquire_task, ex_ctx, &ctx);
        }
        ...
        if (task)
        {
            ...
            if (task->active_coro_nr == 0 && (task->fetched_nr == task->msg_nr))
            {
                ...
                msg_desc_pool.put(task->msg_descs);
                task_pool.put(task);
            }
        }
        ...
        ctx.record_yield_reason(false /* exit */);
        ctx.yield_to_master();
    }
...
{
    auto *rw_ctx = DCHECK_NOTNULL(get_rw_context(ctx));
    rw_ctx->coro_id = ctx ? ctx->coro_id() : kNotACoro;
    rw_ctx->ready = false;
    rw_ctx->inner.dir_id = get_thread_id();
    rw_ctx->inner.wc_status = IBV_WC_SUCCESS;

    WRID o_wrid;
    bool has_work = ex_ctx.commit(rw_ctx, o_wrid);
    if (has_work)
    {
        ctx->record_yield_reason(rw_ctx, o_wrid);
        ctx->yield_to_master();
        DCHECK(rw_ctx->ready);
    }
...
void Patronus::reg_allocator(uint64_t hint,
                             ::mem::IAllocator::pointer allocator)
{
    DCHECK(is_server_)
...
RetCode Patronus::prepare_unprot_write(PatronusBatchContextT &batch,
                                       size_t node_id,
                                       size_t dir_id,
                                       const char *ibuf,
                                       size_t size,
                                       ...
                                       TraceView trace)
{
    ...
    uint32_t rkey = dsm_->get_rkey(node_id, dir_id);

    uint64_t remote_addr = dsm_->buffer_offset_to_dsm_offset(offset);
...
RetCode Patronus::prepare_write(PatronusBatchContextT &batch,
                                Lease &lease,
                                const char *ibuf,
                                size_t size,
                                size_t offset,
...
RetCode Patronus::remote_memcpy(size_t times,
                                size_t size,
                                size_t nid,
                                size_t dir_id,
                                bool offload,
...
RetCode Patronus::prepare_unprot_read(PatronusBatchContextT &batch,
                                      size_t node_id,
                                      size_t dir_id,
                                      char *obuf,
                                      size_t size,
...
RetCode Patronus::prepare_read(PatronusBatchContextT &batch,
                               Lease &lease,
                               char *obuf,
                               size_t size,
                               size_t offset,
...
RetCode Patronus::prepare_unprot_faa(PatronusBatchContextT &batch,
                                     size_t node_id,
                                     size_t dir_id,
                                     char *iobuf,
                                     size_t offset,
...
RetCode Patronus::prepare_faa(PatronusBatchContextT &batch,
                              Lease &lease,
                              char *iobuf,
                              size_t offset,
                              int64_t value,
...
RetCode Patronus::prepare_unprot_cas(PatronusBatchContextT &batch,
                                     size_t node_id,
                                     size_t dir_id,
                                     char *iobuf,
                                     size_t offset,
...
RetCode Patronus::prepare_cas(PatronusBatchContextT &batch,
                              Lease &lease,
                              char *iobuf,
                              size_t offset,
                              uint64_t compare,
...
RetCode Patronus::commit(PatronusBatchContextT &batch,
                         CoroContext *ctx,
                         TraceView trace)
{
    RetCode rc = kOk;
    AsyncContext<RDMA> *rw_context = nullptr;
    ...
    rw_context = DCHECK_NOTNULL(get_rw_context(ctx));
    rw_context->ready = false;
    rw_context->coro_id = ctx ? ctx->coro_id() : kNotACoro;
    rw_context->inner.wc_status = IBV_WC_SUCCESS;
    // We have to fill the below two fields
    // so we retrieve from the batch
    rw_context->inner.target_node = batch.node_id();
    rw_context->inner.dir_id = batch.dir_id();
    rw_context->inner.trace_view = trace;

    rc = batch.commit(rw_context, ctx, trace);

    DCHECK(rw_context->ready)
        << "** When commit finished, should have been ready. rw_ctx at "
        << (void *) rw_context << ", ready at " << (void *) &rw_context->ready
        << ", rw_ctx_id: " << rw_ctx_id << ", coro: " << pre_coro_ctx(ctx);

    wc_status = rw_context->inner.wc_status;
    ...
    bool is_registering_thread)
{
    ...
    {
        LOG(FATAL) << "TODO: deprecated. Adapt patronus's allocator to the one "
                      "in DSM.";
        // auto *dsm_rdma_buffer = desc.dsm_desc.rdma_buffer;
        // auto *dsm_rdma_buffer = nullptr;
        ...
        //         large_message_buffer, large_message_size);
        // char *small_message_buffer = dsm_rdma_buffer + large_message_size;
        // size_t small_message_size = message_pool_size - large_message_size;
        // desc.rdma_small_message_buffer_pool =
        //     std::make_unique<ThreadUnsafeBufferPool<kSmallMessageSize>>(
        // small_message_buffer, small_message_size);
